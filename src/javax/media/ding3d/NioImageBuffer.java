...
public class NioImageBuffer {
    ...
    public void setDataBuffer(Buffer dataBuffer) {
        ...
        if (dataBuffer.limit() != width*height*elementsPerPixel) {
            throw new IllegalArgumentException(Ding3dI18N.getString("NioImageBuffer3"));
        }

        switch (bufferType) {
            case BYTE_BUFFER:
                if (!(dataBuffer instanceof ByteBuffer)) {
                    throw new IllegalArgumentException(Ding3dI18N.getString("NioImageBuffer4"));
                }
            ...
            case INT_BUFFER:
                ...
                if (((IntBuffer)dataBuffer).order() != ByteOrder.nativeOrder()) {
                    throw new IllegalArgumentException(Ding3dI18N.getString("NioImageBuffer5"));
                }
    ...
    private void processParams(int width, int height, ImageType imageType) {
        if (width < 1) {
            throw new IllegalArgumentException(Ding3dI18N.getString("NioImageBuffer0"));
        }

        if (height < 1) {
            throw new IllegalArgumentException(Ding3dI18N.getString("NioImageBuffer1"));
        }
