...
public class CgShaderProgram extends ShaderProgram {
	...
	if (isLiveOrCompiled()) {
		...
		throw new CapabilityNotSetException(Ding3dI18N.getString("CgShaderProgram0"));
	    }
	}
	...
	if (shaders != null) {
            ...
            for (int i = 0; i < shaders.length; i++) {
                ...
                if (shaders[i].getShadingLanguage() != Shader.SHADING_LANGUAGE_CG) {
                    throw new IllegalArgumentException(Ding3dI18N.getString("CgShaderProgram2"));
                }

                // Check for more than one vertex shader or fragment shader
                if (shaders[i].getShaderType() == Shader.SHADER_TYPE_VERTEX) {
                    if (hasVertexShader) {
                        throw new IllegalArgumentException(Ding3dI18N.getString("CgShaderProgram3"));
                    }
                    hasVertexShader = true;
                }
                else { // Shader.SHADER_TYPE_FRAGMENT
                    if (hasFragmentShader) {
                        throw new IllegalArgumentException(Ding3dI18N.getString("CgShaderProgram4"));
                    }
	...
	if (isLiveOrCompiled()) {
		...
		throw new CapabilityNotSetException(Ding3dI18N.getString("CgShaderProgram1"));
	    }
	}
