...
class NativePipeline extends Pipeline {
    ...
    private static final String libPrefix = "Ding3d";
    ...
    static String getSupportedOglVendor() {
        if (!oglChkLibraryLoaded) {
            try {
                loadLibrary("Ding3dcore-ogl-chk", true);
            } catch (Exception ex) {
    ...
    native boolean initializeDing3d(boolean disableXinerama);
...
//KCR:        System.err.println("NativePipeline.getGraphicsConfig()");
    ...
    long getFbConfig(GraphicsConfigInfo gcInfo) {
        ...
        if (fbConfig == 0L) {
            throw new IllegalArgumentException(Ding3dI18N.getString("Canvas3D23"));
        }
