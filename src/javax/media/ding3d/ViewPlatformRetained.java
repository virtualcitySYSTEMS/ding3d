...
class ViewPlatformRetained extends LeafRetained {
	...
	if (source.isLive()) {
	    ...
	    message.type = Ding3dMessage.UPDATE_VIEWPLATFORM;
	    message.threads = Ding3dThread.UPDATE_RENDER|Ding3dThread.UPDATE_BEHAVIOR;
	...
	for (int i = views.length-1; i>=0; i--) {
        ...
        if (inBackgroundGroup) {
            throw new
               IllegalSceneGraphException(Ding3dI18N.getString("ViewPlatformRetained1"));
        }

	if (inSharedGroup) {
	    throw new
		IllegalSharingException(Ding3dI18N.getString("ViewPlatformRetained2"));
	}

	if (s.viewLists != null) {
	    throw new
		IllegalSceneGraphException(Ding3dI18N.getString("ViewPlatformRetained3"));
	}
	/*
	if (false) {
