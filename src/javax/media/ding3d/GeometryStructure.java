...
class GeometryStructure extends Ding3dStructure {
    /**
	...
	super(u, Ding3dThread.UPDATE_GEOMETRY);
	bhNodeCount = 0;
	...
	if (nMsg > 0) {
		...
		case Ding3dMessage.GEOMETRY_CHANGED:
		    // System.err.println("Ding3dMessage.GEOMETRY_CHANGED");
		    processBoundsChanged((Object []) m.args[0], false);
		    break;
	...
	if (bhTreeCount >= bhTreeMax) {
		...
		Ding3dDebug.doDebug(Ding3dDebug.geometryStructure, Ding3dDebug.LEVEL_2,
	...
	if (bhTreeCount == 1) {
	    // For debugging only.
	    if (Ding3dDebug.devPhase) {
		if (Ding3dDebug.doDebug(Ding3dDebug.geometryStructure, Ding3dDebug.LEVEL_2)) {
		...
		for (j=0; j<bhTreeCount; j++) {
			...
			if (Ding3dDebug.devPhase) {
			    Ding3dDebug.doDebug(Ding3dDebug.geometryStructure,
					     Ding3dDebug.LEVEL_2,
     ...
     */
    private void processSwitchChanged(Ding3dMessage m) {
