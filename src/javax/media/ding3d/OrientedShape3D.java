...
public class OrientedShape3D extends Shape3D {
   ...
   // Array for setting default read capabilities
    ...
    public void setAlignmentMode(int mode) {
        if (isLiveOrCompiled())
            if (!this.getCapability(ALLOW_MODE_WRITE))
                throw new CapabilityNotSetException(Ding3dI18N.getString("OrientedShape3D0"));
        if (isLive())
    ...
    public int getAlignmentMode() {
        if (isLiveOrCompiled())
            if (!this.getCapability(ALLOW_MODE_READ))
                throw new CapabilityNotSetException(Ding3dI18N.getString("OrientedShape3D1"));
    ...
    public void getAlignmentAxis(Vector3f axis)  {
        if (isLiveOrCompiled())
            if (!this.getCapability(ALLOW_AXIS_READ))
                throw new CapabilityNotSetException(Ding3dI18N.getString("OrientedShape3D3"));
    ...
    public void setRotationPoint(Point3f point) {
        if (isLiveOrCompiled())
            if (!this.getCapability(ALLOW_POINT_WRITE))
                throw new CapabilityNotSetException(Ding3dI18N.getString("OrientedShape3D4"));
        if (isLive())
    ...
    public void getRotationPoint(Point3f point)  {
        if (isLiveOrCompiled())
            if (!this.getCapability(ALLOW_POINT_READ))
                throw new CapabilityNotSetException(Ding3dI18N.getString("OrientedShape3D5"));
    ...
    public boolean getConstantScaleEnable() {
        if (isLiveOrCompiled())
            if (!this.getCapability(ALLOW_SCALE_READ))
                throw new CapabilityNotSetException(Ding3dI18N.getString("OrientedShape3D7"));
    ...
    public double getScale() {
        if (isLiveOrCompiled())
            if (!this.getCapability(ALLOW_SCALE_READ))
                throw new CapabilityNotSetException(Ding3dI18N.getString("OrientedShape3D9"));
