...
class BehaviorStructure extends Ding3dStructure {
   ...
   /**
	...
	super(u, Ding3dThread.UPDATE_BEHAVIOR);
	...
	if (nMsg > 0) {
		...
		switch (m.type) {
		case Ding3dMessage.TRANSFORM_CHANGED: // Compress Message
		    transformMsg = true;
		    break;
		case Ding3dMessage.COND_MET:
		...
		case Ding3dMessage.REMOVE_NODES:
		    removeNodes(m);
		    break;
		case Ding3dMessage.BEHAVIOR_ACTIVATE:
		    activateBehaviors();
		    break;
		case Ding3dMessage.BEHAVIOR_ENABLE:
		    addToScheduleList((BehaviorRetained) m.args[1]);
		    reEvaluateWakeupCount();
		    break;
		case Ding3dMessage.BEHAVIOR_DISABLE:
		    removeFromScheduleList((BehaviorRetained) m.args[1]);
		    reEvaluateWakeupCount();
		    break;
		case Ding3dMessage.SCHEDULING_INTERVAL_CHANGED:
		...
		case Ding3dMessage.REGION_BOUND_CHANGED:
		    ...
		    break;
		case Ding3dMessage.BEHAVIOR_REEVALUATE:
		    {
	...
	if (activeWakeupOnSensorCount <= 0) {
		...
		VirtualUniverse.mc.sendRunMessage(universe,
						  Ding3dThread.BEHAVIOR_SCHEDULER|
   ...
   /**
	...
	VirtualUniverse.mc.sendRunMessage(universe, Ding3dThread.BEHAVIOR_SCHEDULER);
    }
   ...
   /**
	...
	if ( size > 0) {
	    VirtualUniverse.mc.sendRunMessage(universe,
		      Ding3dThread.BEHAVIOR_SCHEDULER|Ding3dThread.UPDATE_BEHAVIOR);
	}
	...
	if (behav.active) {
	    ...
	    behav.active = false;
	    if (behav.universe != universe) {
		Ding3dMessage m = new Ding3dMessage();
		m.threads = Ding3dThread.UPDATE_BEHAVIOR;
		m.type = Ding3dMessage.BEHAVIOR_REEVALUATE;
