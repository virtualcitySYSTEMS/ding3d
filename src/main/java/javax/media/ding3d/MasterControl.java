...
class MasterControl {
     ...
     * rendering; true by default.  Set by the "Ding3d.usePbuffer"
     ...
     * true by default.
     * Set by the -DDing3d.viewFrustumCulling property, When this flag is
    ...
    private UnorderList stateWorkThreads =
                               new UnorderList(Ding3dThreadData.class);
     ...
     * The current list of work threads
     */
    private UnorderList renderThreadData = new UnorderList(Ding3dThreadData.class);
   ...
   /**
    ...
    // Set by -DDing3d.allowNullGraphicsConfig property
    ...
    // Set by the -DDing3d.sortShape3DBounds property, When this flag is
    ...
    //Set by -DDing3d.forceReleaseView property.
	...
	//awt = Pipeline.getPipeline().getAWT();
        ...
        if (systemStartTime == 0L) {
            systemStartTime = Ding3dClock.currentTimeMillis();
        }

	if(Ding3dDebug.devPhase) {
	    // Check to see whether debug mode is allowed
	    Ding3dDebug.debug = getBooleanProperty("Ding3d.debug", false,
						"Ding3dDebug.debug");
	}
	...
	if (!isD3D()) {
	    isSharedCtx =
		getBooleanProperty("Ding3d.sharedctx", isSharedCtx, "shared contexts");
	}

	doCompaction = getBooleanProperty("Ding3d.docompaction", doCompaction,
					  "compaction");

	// by MIK OF CLASSX
	transparentOffScreen = getBooleanProperty("Ding3d.transparentOffScreen", transparentOffScreen, "transparent OffScreen");

	usePbuffer = getBooleanProperty("Ding3d.usePbuffer",
					usePbuffer,
					"Off-screen Pbuffer");

	viewFrustumCulling = getBooleanProperty("Ding3d.viewFrustumCulling", viewFrustumCulling,"View frustum culling in the renderer is");

	sortShape3DBounds =
	    getBooleanProperty("Ding3d.sortShape3DBounds", sortShape3DBounds,
			       "Shape3D bounds enabled for transparency sorting",
			       "Shape3D bounds *ignored* for transparency sorting");

	forceReleaseView =
	    getBooleanProperty("Ding3d.forceReleaseView", forceReleaseView,
			       "forceReleaseView  after Canvas3D dispose enabled",
			       "forceReleaseView  after Canvas3D dispose disabled");

	useCombiners = getBooleanProperty("Ding3d.usecombiners", useCombiners,
					  "Using NV_register_combiners if available",
					  "NV_register_combiners disabled");

	if (getProperty("Ding3d.disablecompile") != null) {
	    disableCompile = true;
	    System.err.println("Java 3D: BranchGroup.compile disabled");
	}

	if (getProperty("Ding3d.disableSeparateSpecular") != null) {
	    disableSeparateSpecularColor = true;
	    System.err.println("Java 3D: separate specular color disabled if possible");
	}

	isDisplayList = getBooleanProperty("Ding3d.displaylist", isDisplayList,
					   "display list");

	implicitAntialiasing =
	    getBooleanProperty("Ding3d.implicitAntialiasing",
			       implicitAntialiasing,
			       "implicit antialiasing");

	isCompiledVertexArray =
	    getBooleanProperty("Ding3d.compiledVertexArray",
			       isCompiledVertexArray,
			       "compiled vertex array");

        boolean Ding3dOptimizeSpace =
	    getBooleanProperty("Ding3d.optimizeForSpace", true,
			       "optimize for space");

        if (isDisplayList) {
            // Build Display list for by-ref geometry
            // ONLY IF optimizeForSpace is false
            if (!Ding3dOptimizeSpace) {
	...
	Integer threadLimit =
		...
		public Object run() {
		    return Integer.getInteger("Ding3d.threadLimit",
					      defaultThreadLimit);
		}
	    });
	...
	if (cpuLimit < 1)
	    cpuLimit = 1;
	if (Ding3dDebug.debug || cpuLimit != defaultThreadLimit) {
	...
	Integer samplingTime  =
	    ...
	    new java.security.PrivilegedAction() {
		public Object run() {
		    return Integer.getInteger("Ding3d.deviceSampleTime", 0);
		}
	    });
	...
	Integer vattrOffset =
	    ...
	    new java.security.PrivilegedAction() {
		public Object run() {
		    return Integer.getInteger("Ding3d.glslVertexAttrOffset",
	...
	if (Ding3dDebug.debug || glslVertexAttrOffset != defaultGLSLVertexAttrOffset) {
	...
	if (getProperty("Ding3d.disableXinerama") != null) {
	    disableXinerama = true;
	}

        // Issue 480 : Cache bounds returned by getBounds()
        cacheAutoComputedBounds =
                getBooleanProperty("Ding3d.cacheAutoComputeBounds",
                cacheAutoComputedBounds,
                "Cache AutoCompute Bounds, accelerates getBounds()");

        // Issue 544
        useBoxForGroupBounds =
                getBooleanProperty("Ding3d.useBoxForGroupBounds",
                useBoxForGroupBounds,
                "Use of BoundingBox for group geometric bounds");

        // Initialize the native Ding3d library
//	if (!Pipeline.getPipeline().initializeDing3d(disableXinerama)) {
//	    throw new RuntimeException(Ding3dI18N.getString("MasterControl0"));
//	}

	if (xineramaDisabled) {
	    // initializeDing3d() successfully disabled Xinerama.
	    System.err.println("Java 3D: Xinerama disabled");
	}
	else if (disableXinerama) {
	    // Ding3d.disableXinerama is true, but attempt failed.
	    System.err.println("Java 3D: could not disable Xinerama");
	}

        // Check for obsolete properties
        String[] obsoleteProps = {
            "Ding3d.backgroundtexture",
            "Ding3d.forceNormalized",
            "Ding3d.g2ddrawpixel",
            "Ding3d.simulatedMultiTexture",
            "Ding3d.useFreeLists",
        };
...
//	maxLights = Pipeline.getPipeline().getMaximumLights();
    ...
    private static boolean initLogger(Logger logger, Level defaultLevel) {
        ...
        if (defaultLevel != null &&
                ...
                Logger.getLogger("Ding3d").getLevel() == null) {
    ...
    private static void initLoggers() {
        coreLogger = Logger.getLogger("Ding3d.core");
        devLogger = Logger.getLogger("Ding3d.developer");
        statsLogger = Logger.getLogger("Ding3d.stats");
    ...
    static void loadLibraries() {
        ...
        // renderer specified by the "Ding3d.rend" system property.
        //
        ...
        final String rendStr = "jogl";
   //     System.out.println("MasterControl Ding3d.rend "+rendStr);
        ...
        // Get the global Ding3d.shadingLanguage system property
        final String slStr = getProperty("Ding3d.shadingLanguage");
        if (slStr != null) {
            boolean found = false;
   ...
   //         glslLibraryAvailable = Pipeline.getPipeline().isGLSLLibraryAvailable();
     ...
     */
    final boolean isD3D() {
	return false; //Pipeline.getPipeline().getPipelineType() == Pipeline.Type.NATIVE_D3D;
    }
     ...
     */
    void processMessage(Ding3dMessage message) {
     ...
     */
    void processMessage(Ding3dMessage[] messages) {
     ...
     */
    void sendNotification(Ding3dNotification notification) {
     ...
     */
    void sendMessage(Ding3dMessage message) {
 	  ...
 	  synchronized (message) {
	    ...
	    if ((targetThreads & Ding3dThread.UPDATE_RENDERING_ATTRIBUTES) != 0) {
		renderingAttributesStructure.addMessage(message);
	    }

	    // GraphicsContext3D send message with universe = null
	    if (u != null) {
		if ((targetThreads & Ding3dThread.UPDATE_GEOMETRY) != 0) {
		    u.geometryStructure.addMessage(message);
		}
		if ((targetThreads & Ding3dThread.UPDATE_TRANSFORM) != 0) {
		    u.transformStructure.addMessage(message);
		}
		if ((targetThreads & Ding3dThread.UPDATE_BEHAVIOR) != 0) {
		    u.behaviorStructure.addMessage(message);
		}
		if ((targetThreads & Ding3dThread.UPDATE_SOUND) != 0) {
		    u.soundStructure.addMessage(message);
		}
		if ((targetThreads & Ding3dThread.UPDATE_RENDERING_ENVIRONMENT) != 0) {
		    u.renderingEnvironmentStructure.addMessage(message);
		}
	    }

            if ((targetThreads & Ding3dThread.SOUND_SCHEDULER) != 0) {
	    ...
	    if ((targetThreads & Ding3dThread.UPDATE_RENDER) != 0) {
		// Note that we don't check for active view
	...
	synchronized (timeLock) {
	    ...
	    if ((targetThreads & Ding3dThread.INPUT_DEVICE_SCHEDULER) != 0) {
	    ...
	    if ((targetThreads & Ding3dThread.RENDER_THREAD) != 0) {
		synchronized (renderThreadData) {
		    Ding3dThreadData[] threads = (Ding3dThreadData [])
			renderThreadData.toArray(false);
		    int i=renderThreadData.arraySize()-1;
		    Ding3dThreadData thr;
		    while (i>=0) {
			thr = threads[i--];
	...
	synchronized (timeLock) {
	    ...
	    if ((targetThreads & Ding3dThread.BEHAVIOR_SCHEDULER) != 0) {
    ...
    private void updateTimeValues() {
        synchronized (timeLock) {
            int i=0;
            Ding3dThreadData lastThread=null;
            Ding3dThreadData thread=null;
            ...
            while (i<lastTransformStructureThread) {
                ...
                if ((thread.lastUpdateTime > thread.lastRunTime) &&
                    ...
                    thread.threadOpts = Ding3dThreadData.CONT_THREAD;
            ...
            if (lastThread != null) {
                lastThread.threadOpts =  Ding3dThreadData.WAIT_ALL_THREADS;
                lastThread = null;
            }
            ...
            threads = (Ding3dThreadData []) renderWorkThreads.toArray(false);
            size = renderWorkThreads.arraySize();
            View v;
            Ding3dThreadData lastRunThread = null;
            waitTimestamp++;
            sleepTime = 0L;
                    ...
                    if (thread.thread.lastWaitTimestamp == waitTimestamp) {
                        ...
                        if (thread.view != v) {
                            ...
                            lastRunThread.threadOpts =
                                (Ding3dThreadData.STOP_TIMER |
                                 Ding3dThreadData.WAIT_ALL_THREADS);
                            ((Object []) lastRunThread.threadArgs)[3] = lastRunThread.view;
                            thread.threadOpts = (Ding3dThreadData.START_TIMER |
                                                 Ding3dThreadData.CONT_THREAD);
                        } else {
                            if ((lastRunThread.threadOpts &
                                 Ding3dThreadData.START_TIMER) != 0) {
                                lastRunThread.threadOpts =
                                    (Ding3dThreadData.START_TIMER |
                                     Ding3dThreadData.WAIT_ALL_THREADS);
            ...
            if (lastRunThread != null) {
                lastRunThread.threadOpts =
                                    (Ding3dThreadData.STOP_TIMER |
                                     Ding3dThreadData.WAIT_ALL_THREADS|
                                     Ding3dThreadData.LAST_STOP_TIMER);
                lockGeometry = true;
                ((Object []) lastRunThread.threadArgs)[3] = lastRunThread.view;
            }  else {
                lockGeometry = false;
    ...
    private void createUpdateThread(Ding3dStructure structure) {
	final Ding3dStructure s = structure;
    ...
    private void emptyMessageList(Ding3dStructure structure, View v) {
	if (structure != null) {
	    if (v == null) {
		...
		int size = mlist.size();
		if (size > 0) {
		    Ding3dMessage mess[] = (Ding3dMessage []) mlist.toArray(false);
		    Ding3dMessage m;
		    int i = 0;
			...
			if ((v == null) || (m.view == v) ||
			    ...
			    if (m.type == Ding3dMessage.INSERT_NODES) {
    ...
    private void destroyUpdateThread(Ding3dStructure structure) {
	...
	Ding3dThreadData threadData = null;
	...
	if (univ.behaviorScheduler == null) {
	    ...
	    threadData.threadType = Ding3dThread.BEHAVIOR_SCHEDULER;
	...
	if(Ding3dDebug.devPhase) {
	    Ding3dDebug.doDebug(Ding3dDebug.masterControl, Ding3dDebug.LEVEL_1,
    ...
    final private void addToStateThreads(Ding3dThreadData threadData) {
   ...
   /**
	...
	for (i=inputDeviceThreads.size()-1; i >=0; i--) {
	    Ding3dThreadData threadData = ds[i].getThreadData();
	...
	if (thread != null) {
	    thread.threadOpts |= Ding3dThreadData.WAIT_ALL_THREADS;
	}
	...
	Ding3dThreadData threads[];
	int size = 0;

	for (int k=0; k<3; k++) {
	    switch (k) {
	    case 0:
		threads = (Ding3dThreadData []) stateWorkThreads.toArray(false);
		...
		break;
	    default:
		threads = (Ding3dThreadData []) requestRenderWorkThreads.toArray(false);
		size = requestRenderWorkThreads.arraySize();
		break;
	    }

	    for (int i=0; i<size; i++) {
		Ding3dThreadData thread = threads[i];
	...
	Ding3dMessage renderMessage = new Ding3dMessage();
	renderMessage.threads = Ding3dThread.RENDER_THREAD;
	renderMessage.type = Ding3dMessage.RENDER_IMMEDIATE;
	...
	synchronized (mcThreadLock) {
	    ...
	    createMessage.threads = Ding3dThread.RENDER_THREAD;
	    createMessage.type = Ding3dMessage.DESTROY_CTX_AND_OFFSCREENBUFFER;
	...
	GraphicsDevice gd = c.graphicsConfiguration.getDevice();
	Ding3dDebug.doAssert((Screen3D.deviceRendererMap.get(gd) != null),
	...
	synchronized (mcThreadLock) {
	    ...
	    Ding3dMessage createMessage = new Ding3dMessage();
	    createMessage.threads = Ding3dThread.RENDER_THREAD;
	    createMessage.type = Ding3dMessage.CREATE_OFFSCREENBUFFER;
	    createMessage.universe = null;
	    createMessage.view = null;
	    createMessage.args[0] = c;
	    rdr.rendererStructure.addMessage(createMessage);
    ...
    void sendAllocateCanvasId(Canvas3D c) {
        synchronized (mcThreadLock) {
            ...
            Ding3dMessage createMessage = new Ding3dMessage();
            createMessage.threads = Ding3dThread.RENDER_THREAD;
            createMessage.type = Ding3dMessage.ALLOCATE_CANVASID;
            createMessage.universe = null;
            createMessage.view = null;
            createMessage.args[0] = c;
            rdr.rendererStructure.addMessage(createMessage);
    ...
    void sendFreeCanvasId(Canvas3D c) {
        synchronized (mcThreadLock) {
            ...
            Ding3dMessage createMessage = new Ding3dMessage();
            createMessage.threads = Ding3dThread.RENDER_THREAD;
            createMessage.type = Ding3dMessage.FREE_CANVASID;
            createMessage.universe = null;
            createMessage.view = null;
            createMessage.args[0] = c;
            rdr.rendererStructure.addMessage(createMessage);
	...
	for (int i=0; i < size; i++) {
		...
		sendRunMessage(v, Ding3dThread.UPDATE_RENDER);
		threadListsChanged = true;
		rendererRun = true;
	...
	case RUN_THREADS:
	    ...
	    boolean done;
	    Ding3dThreadData thread;
	    Ding3dThreadData renderThreads[] = (Ding3dThreadData [])
		                             renderThreadList.toArray(false);
	    Ding3dThreadData stateThreads[] = (Ding3dThreadData [])
		                            stateThreadList.toArray(false);
	    Ding3dThreadData requestRenderThreads[] = (Ding3dThreadData [])
		...
		while (!renderWaiting &&
		    ...
		    if ((thread.threadOpts & Ding3dThreadData.START_TIMER) != 0) {
		        view = (View)((Object[])thread.threadArgs)[2];
		        view.frameNumber++;
		        view.startTime = Ding3dClock.currentTimeMillis();
		    }
			...
			threadPending++;
			thread.thread.runMonitor(Ding3dThread.RUN,
						 currentTime,
						 (Object[])thread.threadArgs);
		    }

		    if ((thread.threadOpts & Ding3dThreadData.STOP_TIMER) != 0) {
			view = (View)((Object[])thread.threadArgs)[3];
			timestampUpdateList.add(view);
		    }

		    if ((thread.threadOpts & Ding3dThreadData.LAST_STOP_TIMER) != 0) {
		    ...
		    if ((cpuLimit != 1) &&
			(thread.threadOpts &
			 Ding3dThreadData.WAIT_ALL_THREADS) != 0) {
	...
	case THREAD_DONE:
		...
		threadPending--;
                ...
                if (nthread.type == Ding3dThread.RENDER_THREAD) {
		    View v = (View) nthread.args[3];
		    if (v != null) { // STOP_TIMER
			v.stopTime = Ding3dClock.currentTimeMillis();
		    }
	...
	case RUN_RENDERER_CLEANUP:
	    nthread.runMonitor(Ding3dThread.RUN, currentTime,
			       rendererCleanupArgs);
	...
	//dump Threads type
        if ((threads & Ding3dThread.BEHAVIOR_SCHEDULER) != 0) {
            strBuf.append(" BEHAVIOR_SCHEDULER");
        }
        if ((threads & Ding3dThread.SOUND_SCHEDULER) != 0) {
            strBuf.append(" SOUND_SCHEDULER");
        }
        if ((threads & Ding3dThread.INPUT_DEVICE_SCHEDULER) != 0) {
            strBuf.append(" INPUT_DEVICE_SCHEDULER");
        }
        if ((threads & Ding3dThread.RENDER_THREAD) != 0) {
            strBuf.append(" RENDER_THREAD");
        }
        if ((threads & Ding3dThread.UPDATE_GEOMETRY) != 0) {
            strBuf.append(" UPDATE_GEOMETRY");
        }
        if ((threads & Ding3dThread.UPDATE_RENDER) != 0) {
            strBuf.append(" UPDATE_RENDER");
        }
        if ((threads & Ding3dThread.UPDATE_BEHAVIOR) != 0) {
            strBuf.append(" UPDATE_BEHAVIOR");
        }
        if ((threads & Ding3dThread.UPDATE_SOUND) != 0) {
            strBuf.append(" UPDATE_SOUND");
        }
        if ((threads & Ding3dThread.UPDATE_RENDERING_ATTRIBUTES) != 0) {
            strBuf.append(" UPDATE_RENDERING_ATTRIBUTES");
        }
        if ((threads & Ding3dThread.UPDATE_RENDERING_ENVIRONMENT) != 0) {
            strBuf.append(" UPDATE_RENDERING_ENVIRONMENT");
        }
        if ((threads & Ding3dThread.UPDATE_TRANSFORM) != 0) {
    ...
    private void dumpMessage(String str, Ding3dMessage m) {
        StringBuffer strBuf = new StringBuffer();
