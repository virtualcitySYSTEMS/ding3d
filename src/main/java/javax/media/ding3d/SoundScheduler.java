 ...
 * is used for sounds
 */
class SoundScheduler extends Ding3dStructure {
	...
	super(u, Ding3dThread.SOUND_SCHEDULER);
	...
	if (nMsg > 0) {
		...
		case Ding3dMessage.REMOVE_NODES:
		    removeNodes(m);
		    break;
		case Ding3dMessage.SOUND_ATTRIB_CHANGED:
		    changeNodeAttrib(m);
		    break;
		case Ding3dMessage.SOUND_STATE_CHANGED:
		    changeNodeState(m);
		    break;
		case Ding3dMessage.BOUNDINGLEAF_CHANGED:
		    processBoundingLeafChanged(m);
		    break;
		case Ding3dMessage.SOUNDSCAPE_CHANGED:
		    ...
		    break;
		case Ding3dMessage.AURALATTRIBUTES_CHANGED:
		    auralAttribsChanged = true;
		    changeNodeAttrib(m);
		    break;
		case Ding3dMessage.MEDIA_CONTAINER_CHANGED:
		    changeNodeAttrib(m);
		    break;
		case Ding3dMessage.TRANSFORM_CHANGED:
		    ...
		    auralAttribsChanged = true;
		    break;
		case Ding3dMessage.RENDER_IMMEDIATE:
...
*/
    ...
    void changeNodeAttrib(Ding3dMessage m) {
    ...
    void changeNodeState(Ding3dMessage m) {
	Object node = m.args[0];
	Object value = m.args[1];
	if (debugFlag)
	...
	synchronized (soundScapes) {
		...
		if (region!=null && viewPlatform.schedSphere.intersect(region)){
			...
			debugPrint(": region of intersection for "+
				   "soundscape "+k+" found at "+Ding3dClock.currentTimeMillis());
		} else {
		    if (debugFlag)
			debugPrint(
				   ": region of intersection for soundscape "+
				   k + " not found at "+ Ding3dClock.currentTimeMillis());
		}
	    }
	}
...
*/
	...
	case SoundSchedulerAtom.UPDATE:
	    ...
	    long currentTime = Ding3dClock.currentTimeMillis();
	...
	*/
	...
	if (id == SoundRetained.NULL_SOUND) {
	    ...
	    //System.err.println(path + ": "+ Ding3dI18N.getString("SoundRetained1"));
	}
	else {
	    if (debugFlag)
    ...
    void processBoundingLeafChanged(Ding3dMessage m) {
