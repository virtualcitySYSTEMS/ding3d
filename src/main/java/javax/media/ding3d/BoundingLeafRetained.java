...
class BoundingLeafRetained extends LeafRetained {
    ...
    int targetThreads = Ding3dThread.UPDATE_RENDERING_ENVIRONMENT |
	                Ding3dThread.UPDATE_RENDER;


    // Target threads for tranform change
    int transformTargetThreads =
    Ding3dThread.UPDATE_RENDERING_ENVIRONMENT | Ding3dThread.UPDATE_GEOMETRY;
	...
	createMessage.type = Ding3dMessage.BOUNDINGLEAF_CHANGED;
	...
	super.doSetLive(s);
        ...
        if (inBackgroundGroup) {
            throw new
               IllegalSceneGraphException(Ding3dI18N.getString("BoundingLeafRetained0"));
        }

	if (inSharedGroup) {
	    throw new
		IllegalSharingException(Ding3dI18N.getString("BoundingLeafRetained1"));
	}
   ...
   */
	...
	if (node.nodeType == NodeRetained.BACKGROUND ||
	    ...
	    transformTargetThreads |= Ding3dThread.UPDATE_RENDER;
	}
	else if (node instanceof BehaviorRetained) {
	    transformTargetThreads |= Ding3dThread.UPDATE_BEHAVIOR;
	    targetThreads |= Ding3dThread.UPDATE_BEHAVIOR;
	}
	else if (node instanceof SoundRetained ||
		 node.nodeType == NodeRetained.SOUNDSCAPE) {
	    transformTargetThreads |= Ding3dThread.UPDATE_SOUND;
	}
	...
	transformTargetThreads = Ding3dThread.UPDATE_RENDERING_ENVIRONMENT;
	targetThreads = Ding3dThread.UPDATE_RENDERING_ENVIRONMENT |
	                Ding3dThread.UPDATE_RENDER;
