...
import java.awt.*;
import javax.media.ding3d.utils.universe.Viewer; // Needed for Support of DVR.
...
//    boolean dvrResizeCompensation = true;
	...
	if (policy != HMD_VIEW &&
	    ...
	    throw new IllegalArgumentException(Ding3dI18N.getString("View0"));
	}
	if(policy == HMD_VIEW) {
	    // Check the following :
	...
	if (policy != PERSPECTIVE_PROJECTION &&
	    ...
	    throw new IllegalArgumentException(Ding3dI18N.getString("View1"));
	}
	synchronized(this) {
	...
	if (!compatibilityModeEnable) {
	    throw new RestrictedAccessException(Ding3dI18N.getString("View2"));
	}
	...
	if (!compatibilityModeEnable) {
	    throw new RestrictedAccessException(Ding3dI18N.getString("View4"));
	}
	...
	if (!compatibilityModeEnable) {
	    throw new RestrictedAccessException(Ding3dI18N.getString("View8"));
	}
    ...
    public void getUserHeadToVworld(Transform3D t) {
        ...
        if( userHeadToVworldEnable ) {
           ...
           throw new RestrictedAccessException(Ding3dI18N.getString("View9"));
        }
    }
	...
	if (activeStatus && isRunning) {
	    ...
	    Ding3dMessage vpMessage = new Ding3dMessage();
	    vpMessage.universe = universe;
	    vpMessage.view = this;
	    vpMessage.type = Ding3dMessage.UPDATE_VIEW;
	    vpMessage.threads = Ding3dThread.UPDATE_RENDER;
	...
	synchronized(canvasList) {
		...
		throw new IllegalSharingException(Ding3dI18N.getString("View10"));
	...
	if (minimumTime < 0L)
	    throw new IllegalArgumentException(Ding3dI18N.getString("View27"));
	...
	if (minFrameCycleTime == 0) {
		...
		(Ding3dClock.currentTimeMillis() - startTime);
	...
	if (physicalBody == null)
	    throw new IllegalStateException(Ding3dI18N.getString("View13"));

	if (physicalEnvironment == null)
	    throw new IllegalStateException(Ding3dI18N.getString("View14"));
    }
	...
	if (inCanvasCallback) {
		...
		for (int i=canvases.size()-1; i>=0; i--) {
			...
			throw new IllegalStateException(Ding3dI18N.getString(s1));
		    }
		}
	    }
	}
	...
	if ((viewPlatform != null) && viewPlatform.isLive()) {
		...
		throw new IllegalStateException(Ding3dI18N.getString(s2));
	    }
	    return true;
	}
	return false;
	...
	if (inCanvasCallback) {
		...
		for (int i= canvases.size()-1; i>=0; i--) {
			...
			throw new
			    IllegalStateException(Ding3dI18N.getString(s1));
		    }
		}
	    }
	}
    ...
    final void updateViewCache() {
        ...
        // DVR support
	// This is a back door in Ding3d to provide DVR support.
...
//	}
	...
	vpMessage.type = Ding3dMessage.UPDATE_VIEW;
	vpMessage.threads =
			    Ding3dThread.SOUND_SCHEDULER |
			    Ding3dThread.UPDATE_RENDER |
			    Ding3dThread.UPDATE_BEHAVIOR;
	vpMessage.args[0] = this;
	synchronized(((ViewPlatformRetained)viewPlatform.retained).sphere) {
	...
	// using new property -DDing3d.forceReleaseView to disable bug fix 4267395
