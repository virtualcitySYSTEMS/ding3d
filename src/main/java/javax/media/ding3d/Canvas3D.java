...
public class Canvas3D extends Canvas {
    ...
    // This boolean informs the Ding3dGraphics2DImpl that the window is resized
    ...
    // into this Canvas3D
    Ding3dGraphics2DImpl graphics2D = null;
    ...
    protected Transform3D modelMatrix = null;
    protected Transform3D projTrans = null;
    ...
    private static synchronized GraphicsConfiguration
        ...
        if (!offScreen && VirtualUniverse.mc.allowNullGraphicsConfig) {
            if (gconfig == null) {
                ...
                System.err.println(Ding3dI18N.getString("Canvas3D7"));
                System.err.println("    " + Ding3dI18N.getString("Canvas3D18"));
        ...
        if (gconfig == null) {
            throw new NullPointerException(Ding3dI18N.getString("Canvas3D19"));
        } else if (!isValidConfig(gconfig)) {
            throw new IllegalArgumentException(Ding3dI18N.getString("Canvas3D17"));
        }
...
//        if (this instanceof javax.media.ding3d.exp.swing.impl.AutoOffScreenCanvas3D) {
...
//        }
        ...
        if (autoOffScreenCanvas3D && !offScreen) {
            throw new IllegalArgumentException(Ding3dI18N.getString("Canvas3D25"));
        }
     ...
     * @since Java 3D 1.2
     */
    public Ding3dGraphics2D getGraphics2D() {
	synchronized(gfxCreationLock) {
	    if (graphics2D == null)
		graphics2D = new Ding3dGraphics2DImpl(this);
	}
	...
	if (manualRendering)
	    throw new IllegalStateException(Ding3dI18N.getString("Canvas3D14"));
	...
	int width, height;
        ...
        if (offScreenRendering)
            throw new RestrictedAccessException(Ding3dI18N.getString("Canvas3D2"));

	// Check that offScreenBufferPending is not already set
	Ding3dDebug.doAssert(!offScreenBufferPending, "!offScreenBufferPending");
	...
	if (buffer != null) {
		...
		throw new IllegalArgumentException(Ding3dI18N.getString("Canvas3D15"));
	    }

	    if (bufferRetained.getNumberOfComponents() < 3 ) {
		throw new IllegalArgumentException(Ding3dI18N.getString("Canvas3D16"));
	    }

            if (buffer.isLive()) {
                throw new IllegalSharingException(Ding3dI18N.getString("Canvas3D26"));
            }

            if (bufferRetained.getInImmCtx()) {
                throw new IllegalSharingException(Ding3dI18N.getString("Canvas3D27"));
            }

            if (buffer != offScreenBuffer && bufferRetained.getUsedByOffScreen()) {
                throw new IllegalSharingException(Ding3dI18N.getString("Canvas3D28"));
            }
    ...
    public void renderOffScreenBuffer() {
        ...
        if (!manualRendering)
            throw new IllegalStateException(Ding3dI18N.getString("Canvas3D24"));

        // Issue 260 : Cannot render if we already have a fatal error
        if (fatalError) {
            throw new IllegalRenderingStateException(Ding3dI18N.getString("Canvas3D30"));
        }

        if (offScreenBuffer == null)
            throw new NullPointerException(Ding3dI18N.getString("Canvas3D10"));

	Dimension screenSize = screen.getSize();

        if (screenSize.width <= 0)
            throw new IllegalStateException(Ding3dI18N.getString("Canvas3D8"));

        if (screenSize.height <= 0)
            throw new IllegalStateException(Ding3dI18N.getString("Canvas3D9"));

        if (screen.getPhysicalScreenWidth() <= 0.0)
            throw new IllegalStateException(Ding3dI18N.getString("Canvas3D12"));

        if (screen.getPhysicalScreenHeight() <= 0.0)
            throw new IllegalStateException(Ding3dI18N.getString("Canvas3D13"));

        if (offScreenRendering)
            throw new RestrictedAccessException(Ding3dI18N.getString("Canvas3D2"));

	if (!isRunning)
            throw new RestrictedAccessException(Ding3dI18N.getString("Canvas3D11"));
	...
	try {
	    ...
	    // is true. This assumption is valid among all Ding3d threads as
		...
		createMessage.type = Ding3dMessage.RENDER_OFFSCREEN;
	    ...
	    // we have to use pendingView instead.
            Ding3dMessage createMessage = new Ding3dMessage();
            createMessage.threads = Ding3dThread.UPDATE_RENDER;
            createMessage.type = Ding3dMessage.RENDER_OFFSCREEN;
	...
	if (isRunning)
	    throw new RestrictedAccessException(Ding3dI18N.getString("Canvas3D0"));
	...
	if (view != null && graphicsContext3D != null) {
	    ...
	    graphicsContext3D.runMonitor(Ding3dThread.WAIT);
	}
    }
	...
	if (firstPaintCalled && useDoubleBuffer) {
		...
		if (validCtx && (ctx != null) && (view != null)) {
			...
			if (validCtx) {
				...
				graphicsContext3D.runMonitor(Ding3dThread.NOTIFY);
				return;
			    }
	...
	if((view !=null) && (view.viewPolicy == View.HMD_VIEW) &&
	   ...
	   (!useStereo)) {
	    throw new
		IllegalStateException(Ding3dI18N.getString("View31"));
	}
     ...
     * Ding3d.textureEnforcePowerOfTwo to revert to the pre-1.5 behavior
	...
	if (queryProps == null) {
        ...
        if (fatalError) {
            throw new IllegalStateException(Ding3dI18N.getString("Canvas3D29"));
        }
	...
	queryProps =
	    new Ding3dQueryProps((String[]) keys.toArray(new String[0]),
				   values.toArray());
    }
	...
	if ((graphics2D != null) &&
	    ...
	    // let Ding3dGraphics2DImpl to initialize texture again
	    graphics2D.objectId = -1;
	}
	...
	if (active) {
		...
		if ((view != null) &&
		    ...
		    if (cmd == RESIZE) {
			renderMessage.type = Ding3dMessage.RESIZE_CANVAS;
		    } else {
			renderMessage.type = Ding3dMessage.TOGGLE_CANVAS;
		    }
    ...
    protected void setModelViewMatrix(Context ctx, double[] viewMatrix, Transform3D mTrans) {
     ...
     * property Ding3d.disableSeparateSpecular.
     */
	...
	throw new UnsupportedOperationException(Ding3dI18N.getString("Canvas3D20"));
    }
...
//            assert id == ((TextureRetained)obj).objectId;
    ...
    protected void setModelViewMatrix(Context ctx, double[] viewMatrix, double[] modelMatrix) {
        Pipeline.getPipeline().setModelViewMatrix(ctx, viewMatrix, modelMatrix);
    }

    // The native method for setting the Projection matrix.
    protected void setProjectionMatrix(Context ctx, double[] projMatrix) {
