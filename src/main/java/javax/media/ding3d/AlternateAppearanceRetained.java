...
class AlternateAppearanceRetained extends LeafRetained {
  ...
  int indexOfScope(Group scope) {
	...
	TransformGroupRetained[] tlist;
        ...
        if (inImmCtx) {
	    throw new IllegalSharingException(Ding3dI18N.getString("AlternateAppearanceRetained13"));
        }

	if (inSharedGroup) {
	    throw new
		IllegalSharingException(Ding3dI18N.getString("AlternateAppearanceRetained15"));
	}

	if (inBackgroundGroup) {
	    throw new
		IllegalSceneGraphException(Ding3dI18N.getString("AlternateAppearanceRetained16"));
	}
	...
	if (appearance != null) {
		...
		throw new IllegalSharingException(Ding3dI18N.getString("AlternateAppearanceRetained14"));
	    }
	    synchronized(appearance.liveStateLock) {
	...
	createMessage.type = Ding3dMessage.ALTERNATEAPPEARANCE_CHANGED;
