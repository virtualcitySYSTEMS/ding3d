...
class SharedGroupRetained extends GroupRetained implements TargetsInterface {
    ...
    */
	...
	s.transformTargets = newTargets;
        ...
        // Assign data in cachedTargets to Ding3dCTs.
        Ding3dCTs = new CachedTargets[cachedTargets.length];
        copyCachedTargets(TargetsInterface.TRANSFORM_TARGETS, Ding3dCTs);
...
// XXXX : port this
        for (int i=0; i < children.size(); i++) {
            if ((childContains[i][0] & ILLEGAL_LEAF_MASK) != 0) {
                throw new IllegalSharingException(Ding3dI18N.getString("SharedGroupRetained0"));            }
        }
*/
    }
	...
	Object obj;
        ...
        CachedTargets ct = Ding3dCTs[index];
        if (ct != null) {
    ...
    void compile()  {
        ...
        if (Ding3dDebug.devPhase && Ding3dDebug.debug) {
            Ding3dDebug.doDebug(Ding3dDebug.compileState, Ding3dDebug.LEVEL_3,
        ...
        if (Ding3dDebug.devPhase && Ding3dDebug.debug) {
            if (Ding3dDebug.doDebug(Ding3dDebug.compileState, Ding3dDebug.LEVEL_3)) {
                compState.printStats();
            }
            if (Ding3dDebug.doDebug(Ding3dDebug.compileState, Ding3dDebug.LEVEL_5)) {
