...
abstract class NodeRetained extends SceneGraphObjectRetained implements NnuId {
	...
	if(nodeR instanceof SharedGroupRetained) {
	    ...
	    // Problem !
	    throw new RuntimeException(Ding3dI18N.getString("NodeRetained4"));
	}
	else {
		...
		if(((BranchGroupRetained)(nodeR)).locale != null) {
		    ...
		    throw new RuntimeException(Ding3dI18N.getString("NodeRetained5"));
		}
	    }
	    else {
	...
	if (inSharedGroup) {
	    throw new IllegalSharingException(Ding3dI18N.getString("NodeRetained0"));
	}
    ...
    void getLocalToVworld(SceneGraphPath path, Transform3D t) {
	    ...
	    throw new IllegalSharingException(Ding3dI18N.getString("NodeRetained1"));
        }
        path.validate(key);
	...
	if(!(locale == null || universe == s.universe))
            throw new IllegalSharingException(Ding3dI18N.getString("NodeRetained3"));
	if(s.locale == null)
