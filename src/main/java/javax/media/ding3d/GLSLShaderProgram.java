...
public class GLSLShaderProgram extends ShaderProgram {
	...
	if (isLiveOrCompiled()) {
		...
		throw new CapabilityNotSetException(Ding3dI18N.getString("GLSLShaderProgram0"));
	    }
	}
        ...
        if(shaders != null) {
            ...
            for (int i = 0; i < shaders.length; i++) {
                if (shaders[i].getShadingLanguage() != Shader.SHADING_LANGUAGE_GLSL) {
                    throw new IllegalArgumentException(Ding3dI18N.getString("GLSLShaderProgram2"));
                }
	...
	if (isLiveOrCompiled()) {
		...
		throw new CapabilityNotSetException(Ding3dI18N.getString("GLSLShaderProgram1"));
	    }
	}
