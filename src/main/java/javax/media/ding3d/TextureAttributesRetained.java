...
class TextureAttributesRetained extends NodeComponentRetained {
	...
	if (table.length < 3 || table.length > 4) {
            throw new IllegalArgumentException(Ding3dI18N.getString("TextureAttributes13"));
	}

	if (Texture.getPowerOf2(table[0].length) == -1) {
            throw new IllegalArgumentException(Ding3dI18N.getString("TextureAttributes14"));
	}

	for (int i = 1; i < table.length; i++) {
	     if (table[i].length != table[0].length)
                 throw new IllegalArgumentException(Ding3dI18N.getString("TextureAttributes15"));
	}
    ...
    */
	...
	createMessage.type = Ding3dMessage.TEXTUREATTRIBUTES_CHANGED;
