...
class RasterRetained extends GeometryRetained {
	...
	position.z = pos.z;
	geomLock.unLock();
        sendChangedMessage(Ding3dThread.UPDATE_GEOMETRY, null, null);
    }
    ...
    final void setImage(ImageComponent2D img) {
        ...
        if (source.isLive()) {
            ...
            sendChangedMessage((Ding3dThread.UPDATE_RENDER|Ding3dThread.UPDATE_RENDERING_ATTRIBUTES),
                    oldTex, this.texture);
        }
    }
    ...
    */
	...
	synchronized(liveStateLock) {
		...
		synchronized (universeList) {
		    ...
		    Ding3dMessage[] m = new Ding3dMessage[numMessages];
		    for (int i=0; i<numMessages; i++) {
			m[i] = new Ding3dMessage();
			m[i].type = Ding3dMessage.GEOMETRY_CHANGED;
			m[i].threads = threads;
