...
class Renderer extends Ding3dThread {
	...
	Renderer(ThreadGroup t) {
		super(t);
		setName("Ding3d-Renderer-" + getInstanceNum());

		type = Ding3dThread.RENDER_THREAD;
		rendererId = VirtualUniverse.mc.getRendererId();
		rendererBit = (1 << rendererId);
		renderMessage = new Ding3dMessage[1];
	}
	...
	void doWork(long referenceTime) {
		...
		try {
			if (opArg == SWAP) {
					...
					doneSwap: try {
						...
						RenderingError err =
							new RenderingError(RenderingError.UNEXPECTED_RENDERING_ERROR,
									Ding3dI18N.getString("Renderer0"));
						err.setCanvas3D(cv);
				...
				if (opArg == RENDER) {
					m = renderMessage;
					m[0] = new Ding3dMessage();
					// Issue 131: Set appropriate message type
					if (((Canvas3D)args[1]).offScreen) {
						m[0].type = Ding3dMessage.RENDER_OFFSCREEN;
					}
					else {
						m[0].type = Ding3dMessage.RENDER_RETAINED;
					}
					m[0].incRefcount();
					m[0].args[0] = args[1];
					totalMessages = 1;
				...
				doneRender: while (nmesg < totalMessages) {
						...
						if (secondArg instanceof Canvas3D) {
								...
								try {
									...
									RenderingError err =
										new RenderingError(RenderingError.GRAPHICS_CONFIG_ERROR,
												Ding3dI18N.getString("Renderer1"));
									err.setCanvas3D(c);
									err.setGraphicsDevice(c.graphicsConfiguration.getDevice());
									notifyErrorListeners(err);
								}
								GraphicsConfigTemplate3D.runMonitor(Ding3dThread.NOTIFY);
								...
								try {
									...
									RenderingError err =
										new RenderingError(RenderingError.CONTEXT_CREATION_ERROR,
												Ding3dI18N.getString("Renderer2"));
									err.setCanvas3D(c);
									err.setGraphicsDevice(c.graphicsConfiguration.getDevice());
									notifyErrorListeners(err);
								}
							...
							if (reqType == MasterControl.GETBESTCONFIG) {
								...
								try {
									...
									RenderingError err =
										new RenderingError(RenderingError.GRAPHICS_CONFIG_ERROR,
												Ding3dI18N.getString("Renderer3"));
								...
								try {
									...
									RenderingError err =
										new RenderingError(RenderingError.GRAPHICS_CONFIG_ERROR,
												Ding3dI18N.getString("Renderer4"));
							...
							gct.runMonitor(Ding3dThread.NOTIFY);
						}
					...
					if (renderType == Ding3dMessage.CREATE_OFFSCREENBUFFER) {
						// Fix for issue 18.
						// Fix for issue 20.
						...
						if (canvas.drawable == null) {
							...
							RenderingError err =
								new RenderingError(RenderingError.OFF_SCREEN_BUFFER_ERROR,
										Ding3dI18N.getString("Renderer5"));
					...
					else if (renderType == Ding3dMessage.DESTROY_CTX_AND_OFFSCREENBUFFER) {
						...
						m[nmesg++].decRefcount();
						continue;
					} else if (renderType == Ding3dMessage.ALLOCATE_CANVASID) {
						canvas.allocateCanvasId();
					} else if (renderType == Ding3dMessage.FREE_CANVASID) {
						canvas.freeCanvasId();
					}

					if ((canvas.view == null) || !canvas.firstPaintCalled) {
						// This happen when the canvas just remove from the View
						if (renderType == Ding3dMessage.RENDER_OFFSCREEN) {
					...
					if (!canvas.validCanvas &&
							(renderType != Ding3dMessage.RENDER_OFFSCREEN)) {
						m[nmesg++].decRefcount();
						continue;
					}

					if (renderType == Ding3dMessage.RESIZE_CANVAS) {
						canvas.d3dResize();
						// render the image again after resize
						VirtualUniverse.mc.sendRunMessage(canvas.view, Ding3dThread.RENDER_THREAD);
						m[nmesg++].decRefcount();
					} else if (renderType == Ding3dMessage.TOGGLE_CANVAS) {
						canvas.d3dToggle();
						VirtualUniverse.mc.sendRunMessage(canvas.view, Ding3dThread.RENDER_THREAD);
						m[nmesg++].decRefcount();
					} else if (renderType == Ding3dMessage.RENDER_IMMEDIATE) {
						...
						try {
							...
							RenderingError err =
								new RenderingError(RenderingError.CONTEXT_CREATION_ERROR,
										Ding3dI18N.getString("Renderer6"));
							err.setCanvas3D(canvas);
							err.setGraphicsDevice(canvas.graphicsConfiguration.getDevice());
							notifyErrorListeners(err);
						}
							...
							if (sharedCtx == null) {
								...
								synchronized (VirtualUniverse.mc.contextCreationLock) {
									...
									if (sharedCtx == null) {
										...
										RenderingError err =
											new RenderingError(RenderingError.CONTEXT_CREATION_ERROR,
													Ding3dI18N.getString("Renderer7"));
										err.setCanvas3D(canvas);
										err.setGraphicsDevice(canvas.graphicsConfiguration.getDevice());
										notifyErrorListeners(err);
			...
			RenderingError err =
				new RenderingError(RenderingError.UNEXPECTED_RENDERING_ERROR,
						Ding3dI18N.getString("Renderer8"));
			err.setCanvas3D(canvas);
			if (canvas != null) {
				err.setGraphicsDevice(canvas.graphicsConfiguration.getDevice());
			}
	...
	static void notifyErrorListeners(RenderingError err) {
		Ding3dNotification notification = new Ding3dNotification();
		notification.type = Ding3dNotification.RENDERING_ERROR;
