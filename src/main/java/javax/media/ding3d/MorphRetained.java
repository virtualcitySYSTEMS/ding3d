...
class MorphRetained extends LeafRetained implements GeometryUpdater {
    ...
    final static int targetThreads = (Ding3dThread.UPDATE_RENDER |
	...
	if (source.isLive()) {
	    ...
	    Ding3dMessage message = new Ding3dMessage();
	    message.type = Ding3dMessage.COLLISION_BOUND_CHANGED;
            message.threads = Ding3dThread.UPDATE_TRANSFORM;
	...
	if (source.isLive() && !boundsAutoCompute) {
	    Ding3dMessage message = new Ding3dMessage();
	    message.type = Ding3dMessage.REGION_BOUND_CHANGED;
	    message.threads = Ding3dThread.UPDATE_TRANSFORM |
		              targetThreads;
	    message.universe = universe;
	...
	if ((geometryArrays == null || geometryArrays.length == 0) && numGeometryArrays == 0)
	    ...
	    throw new IllegalArgumentException(Ding3dI18N.getString("MorphRetained0"));


        for (i=1;i < geometryArrays.length;i++) {
	    if (geometryArrays[i] == null || geometryArrays[i-1] == null)
		throw new IllegalArgumentException(Ding3dI18N.getString("MorphRetained1"));
	...
	if ((geo.vertexFormat & GeometryArray.VERTEX_ATTRIBUTES) != 0) {
	    throw new UnsupportedOperationException(Ding3dI18N.getString("MorphRetained9"));
	}
	...
	if (source.isLive()) {
	    ...
	    Ding3dMessage mChangeMessage = null;
	    mChangeMessage = new Ding3dMessage();
	    mChangeMessage.type = Ding3dMessage.MORPH_CHANGED;
	    mChangeMessage.threads = (Ding3dThread.UPDATE_GEOMETRY |
				      Ding3dThread.UPDATE_TRANSFORM);
	    // If its a indexed geometry array, unindexify in renderBin
	    if (this.geometryArrays[0] instanceof IndexedGeometryArrayRetained)
		mChangeMessage.threads |= Ding3dThread.UPDATE_RENDERING_ATTRIBUTES;
	...
	boolean visibleIsDirty = false;
		...
		size = 1;
	    Ding3dMessage[] createMessage = new Ding3dMessage[size];
	    createMessage[0] = new Ding3dMessage();
	    createMessage[0].threads = Ding3dThread.UPDATE_RENDERING_ENVIRONMENT |
		Ding3dThread.UPDATE_RENDER;
	    createMessage[0].type = Ding3dMessage.MORPH_CHANGED;
	    ...
	    if(visibleIsDirty) {
		createMessage[1] = new Ding3dMessage();
		createMessage[1].threads = Ding3dThread.UPDATE_GEOMETRY;
		createMessage[1].type = Ding3dMessage.SHAPE3D_CHANGED;
    ...
    void setAppearanceOverrideEnable(boolean flag) {
	    ...
	    createMessage.threads = Ding3dThread.UPDATE_RENDERING_ENVIRONMENT |
		Ding3dThread.UPDATE_RENDER;;
	    createMessage.type = Ding3dMessage.MORPH_CHANGED;
	...
	if (localToVworld == null) {
	    throw new RuntimeException(Ding3dI18N.getString("MorphRetained5"));
	}
	...
	if (weights.length != numGeometryArrays)
	    throw new IllegalArgumentException(Ding3dI18N.getString("MorphRetained7"));
	...
	if (Math.abs(sum - 1.0) > TOLERANCE)
	    throw new IllegalArgumentException(Ding3dI18N.getString("MorphRetained8"));
	...
	s.notifyThreads |= (Ding3dThread.UPDATE_GEOMETRY |
			    Ding3dThread.UPDATE_TRANSFORM |
			    Ding3dThread.UPDATE_RENDER |
			    Ding3dThread.UPDATE_RENDERING_ATTRIBUTES);

	// Need to clone the geometry , if its indexed ...
	if (refCount == 1 && this.geometryArrays[0] instanceof IndexedGeometryArrayRetained) {
	    Ding3dMessage mChangeMessage = new Ding3dMessage();
	    mChangeMessage.type = Ding3dMessage.MORPH_CHANGED;
	    mChangeMessage.threads = Ding3dThread.UPDATE_RENDERING_ATTRIBUTES;
	    mChangeMessage.args[0] = this;
	    mChangeMessage.args[1]= new Integer(GEOMETRY_CHANGED);
	...
	s.notifyThreads |= (Ding3dThread.UPDATE_GEOMETRY |
			    ...
			    Ding3dThread.UPDATE_RENDERING_ENVIRONMENT |
			    Ding3dThread.UPDATE_RENDER);
    ...
    void setBoundsAutoCompute(boolean autoCompute) {
	    ...
	    localBounds = getBounds();
            ...
            if (source.isLive()) {
                Ding3dMessage message = new Ding3dMessage();
                message.type = Ding3dMessage.BOUNDS_AUTO_COMPUTE_CHANGED;
                message.threads = Ding3dThread.UPDATE_TRANSFORM |
		                  Ding3dThread.UPDATE_GEOMETRY |
		                  Ding3dThread.UPDATE_RENDER;
                message.universe = universe;
