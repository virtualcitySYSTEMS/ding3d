...
public class LineAttributes extends NodeComponent {
   ...
   // Array for setting default read capabilities
     ...
     */
	 ...
	 throw new IllegalArgumentException(Ding3dI18N.getString("LineAttributes0"));
    ...
    public void setLineWidth(float lineWidth) {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_WIDTH_WRITE))
              throw new CapabilityNotSetException(Ding3dI18N.getString("LineAttributes1"));
	if (isLive())
    ...
    public float getLineWidth() {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_WIDTH_READ))
              throw new CapabilityNotSetException(Ding3dI18N.getString("LineAttributes2"));
    ...
    public void setLinePattern(int linePattern) {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_PATTERN_WRITE))
              throw new CapabilityNotSetException(Ding3dI18N.getString("LineAttributes3"));

        if (linePattern < PATTERN_SOLID || linePattern > PATTERN_USER_DEFINED)
	  throw new IllegalArgumentException(Ding3dI18N.getString("LineAttributes4"));
    ...
    public int getLinePattern() {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_PATTERN_READ))
              throw new CapabilityNotSetException(Ding3dI18N.getString("LineAttributes5"));
    ...
    public int getPatternMask() {
	    ...
	    throw new CapabilityNotSetException(Ding3dI18N.getString("LineAttributes9"));
	...
	if (isLiveOrCompiled() &&
	    !this.getCapability(ALLOW_PATTERN_WRITE))
	    throw new CapabilityNotSetException(Ding3dI18N.getString("LineAttributes10"));
	...
	if (isLiveOrCompiled() &&
            !this.getCapability(ALLOW_PATTERN_READ))
	    throw new CapabilityNotSetException(Ding3dI18N.getString("LineAttributes11"));
    ...
    public boolean getLineAntialiasingEnable() {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_ANTIALIASING_READ))
              throw new CapabilityNotSetException(Ding3dI18N.getString("LineAttributes7"));
