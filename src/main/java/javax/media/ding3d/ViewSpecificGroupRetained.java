   ...
   // The object that contains the dynamic HashKey - a string type object
	...
	if (source.isLive() && view != null) {
		...
		for (int k = 0; k < localToVworldKeys.length; k++) {
			...
			*/
			...
			message.type = Ding3dMessage.VIEWSPECIFICGROUP_CHANGED;
			message.threads = (Ding3dThread.UPDATE_RENDERING_ENVIRONMENT|
					   Ding3dThread.UPDATE_RENDER |
					   Ding3dThread.UPDATE_SOUND|
    ...
    void setLive(SetLiveState s) {
        if (inBackgroundGroup) {
            throw new
               IllegalSceneGraphException(Ding3dI18N.getString("ViewSpecificGroup3"));
        }
