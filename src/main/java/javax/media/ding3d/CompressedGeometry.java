...
public class CompressedGeometry extends Geometry {
	...
	if ((hdr.size + hdr.start) > compressedGeometry.length)
		...
		(Ding3dI18N.getString("CompressedGeometry0")) ;
	...
	// non-Ding3d-specific metadata.  Only one of these blocks is used per
	...
	throw new UnsupportedOperationException(Ding3dI18N.getString("CompressedGeometry9")) ;
    }
	...
	if (isLiveOrCompiled())
	  if (!this.getCapability(ALLOW_COUNT_READ))
		...
		(Ding3dI18N.getString("CompressedGeometry1")) ;
	...
	if (isLiveOrCompiled())
	  if (!this.getCapability(ALLOW_HEADER_READ))
	    throw new CapabilityNotSetException
		(Ding3dI18N.getString("CompressedGeometry2")) ;
	...
	if (isLiveOrCompiled())
	  if (!this.getCapability(ALLOW_GEOMETRY_READ))
	    throw new CapabilityNotSetException
		(Ding3dI18N.getString("CompressedGeometry3")) ;

	if (isByReference())
	    throw new IllegalStateException
		(Ding3dI18N.getString("CompressedGeometry7")) ;

	if (cgHeader.size > compressedGeometry.length)
	    throw new ArrayIndexOutOfBoundsException
		(Ding3dI18N.getString("CompressedGeometry4")) ;
	...
	if (isLiveOrCompiled())
	  if (!this.getCapability(ALLOW_GEOMETRY_READ))
	    throw new CapabilityNotSetException
		(Ding3dI18N.getString("CompressedGeometry5")) ;
	...
	if (isLiveOrCompiled())
		...
		throw new CapabilityNotSetException
		    (Ding3dI18N.getString("CompressedGeometry6")) ;

	if (!isByReference())
	    throw new IllegalStateException
		(Ding3dI18N.getString("CompressedGeometry8")) ;
