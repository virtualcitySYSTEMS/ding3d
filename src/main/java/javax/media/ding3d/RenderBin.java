...
class RenderBin extends Ding3dStructure  implements ObjectUpdate {
	...
	RenderBin(VirtualUniverse u, View v) {
		super(u, Ding3dThread.UPDATE_RENDER);
	...
	public void updateObject() {
		...
		if(size > 0) {
			Ding3dMessage m;
			for(i=0; i<size; i++) {
				m = (Ding3dMessage) ogCIOList.get(i);

				switch(m.type) {
				case Ding3dMessage.ORDERED_GROUP_TABLE_CHANGED:
				...
				case Ding3dMessage.ORDERED_GROUP_INSERTED:
				case Ding3dMessage.ORDERED_GROUP_REMOVED:
					if(m.args[3] != null) {
		...
		if (tbUpdateList.size() > 0) {
			...
			for (i = 0; i < size; i++) {
				...
				// not equal to null. This is a temporary fix for Ding3d1.3.
		...
		if ( size > 0) {
			Ding3dMessage m;
			for (i=size-1; i>=0; i--) {
				m = (Ding3dMessage) offScreenMessage.get(i);
				m.threads = Ding3dThread.RENDER_THREAD;
	...
	void processMessages(long referenceTime) {
		...
		if (nMsg > 0) {
			for (i=0; i < nMsg; i++) {
				m = messages[i];
				switch (m.type) {
				case Ding3dMessage.INSERT_NODES:
					insertNodes(m);
					m.decRefcount();
					break;
				case Ding3dMessage.REMOVE_NODES:
					removeNodes(m);
					m.decRefcount();
					break;
				case  Ding3dMessage.TRANSFORM_CHANGED:
					transformMsg = true;
					m.decRefcount();
					break;
				case Ding3dMessage.LIGHT_CHANGED:
					...
					break;
				case Ding3dMessage.SWITCH_CHANGED:
					visGAIsDirty = true;
					visQuery = true;
					...
					m.decRefcount();
					break;
				case Ding3dMessage.BACKGROUND_CHANGED:
					...
					m.decRefcount();
					break;
				case Ding3dMessage.CLIP_CHANGED:
				...
				case Ding3dMessage.APPEARANCE_CHANGED:
					...
					m.decRefcount();
					break;
				case Ding3dMessage.FOG_CHANGED:
					...
					m.decRefcount();
					break;
				case Ding3dMessage.ALTERNATEAPPEARANCE_CHANGED:
					...
					m.decRefcount();
					break;
				case Ding3dMessage.MODELCLIP_CHANGED:
					...
					m.decRefcount();
					break;
				case Ding3dMessage.BOUNDINGLEAF_CHANGED:
					processBoundingLeafChanged(m.args,
							referenceTime);
					m.decRefcount();
					break;
				case Ding3dMessage.SHAPE3D_CHANGED:
					processShapeChanged(m.args, referenceTime);
					m.decRefcount();
					break;
				case Ding3dMessage.ORIENTEDSHAPE3D_CHANGED:
				...
				case Ding3dMessage.MORPH_CHANGED:
					...
					m.decRefcount();
					break;
				case Ding3dMessage.UPDATE_VIEW:
				{
				...
				break;
				case Ding3dMessage.UPDATE_VIEWPLATFORM:
					updateViewPlatform((ViewPlatformRetained) m.args[0],
							((Float)m.args[1]).floatValue());
					m.decRefcount();
					break;
				case Ding3dMessage.TEXT3D_DATA_CHANGED:
				...
				case Ding3dMessage.BOUNDS_AUTO_COMPUTE_CHANGED:
				case Ding3dMessage.REGION_BOUND_CHANGED:
					processGeometryAtomsChanged((Object[])m.args[0]);
					visGAIsDirty = true;
					visQuery = true;
					m.decRefcount();
					break;
				case Ding3dMessage.TEXT3D_TRANSFORM_CHANGED:
				...
				case Ding3dMessage.ORDERED_GROUP_TABLE_CHANGED:
					...
					ogCIOList.add(m);
					break;
				case Ding3dMessage.RENDER_OFFSCREEN:
					offScreenMessage.add(m);
					break;
				case Ding3dMessage.VIEWSPECIFICGROUP_CHANGED:
	...
	void processSwitchChanged(Ding3dMessage m, long refTime) {
		int i;
		UnorderList arrList;
		int size;
...
System.err.println("......tb.soleUser= " +
	...
	private void processOrderedGroupRemoved(Ding3dMessage m) {
		int i, n;
	...
	private void processOrderedGroupInserted(Ding3dMessage m) {
		Object[] ogList = (Object[])m.args[0];
		Object[] ogChildIdList = (Object[])m.args[1];
	...
	private void processLightChanged() {
		...
		for (n = 0; n < lightSize; n++) {
			Ding3dMessage msg = (Ding3dMessage)lightMessageList.get(n);
			args = msg.args;
	...
	TransparentRenderingInfo depthSortAll(TransparentRenderingInfo startinfo) {
		transparencySortComparator = javax.media.ding3d.utils.scenegraph.transparency.TransparencySortController.getComparator(view);
	...
	void processViewSpecificGroupChanged(Ding3dMessage m) {
	...
	void insertNodes(Ding3dMessage m) {
		Object nodes[];
	...
	/*
    void handleFrequencyBitChanged(Ding3dMessage m) {
	NodeComponentRetained nc = (NodeComponentRetained)m.args[0];
	GeometryAtom[] gaArr = (GeometryAtom[])m.args[3];
	int i;
	RenderAtom ra;
