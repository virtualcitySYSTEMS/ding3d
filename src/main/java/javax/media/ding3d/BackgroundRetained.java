...
class BackgroundRetained extends LeafRetained {
    ...
    final static int targetThreads = Ding3dThread.UPDATE_RENDERING_ENVIRONMENT |
    ...
    final void setGeometry(BranchGroup branch) {
	    ...
	    Ding3dMessage m[];
            ...
            m = new Ding3dMessage[numMessages];
            for (i=0; i<numMessages; i++) {
                m[i] = new Ding3dMessage();
            }
            i = 0;
            if (geometryBranch != null) {
                clearGeometryBranch((BranchGroupRetained)geometryBranch.retained);
                m[i].threads = (Ding3dThread.UPDATE_RENDER |
				Ding3dThread.UPDATE_RENDERING_ENVIRONMENT);
                m[i].type = Ding3dMessage.ORDERED_GROUP_REMOVED;
		...
		m[i].args[4] = setLiveState.ogCIOTableList.toArray();
                ...
                m[i].type = Ding3dMessage.REMOVE_NODES;
                m[i].universe = universe;
            ...
            if (branch != null) {
                ...
                m[i].threads = (Ding3dThread.UPDATE_RENDER |
				Ding3dThread.UPDATE_RENDERING_ENVIRONMENT);
                m[i].type = Ding3dMessage.ORDERED_GROUP_INSERTED;
                m[i].universe = universe;
                m[i].args[0] = setLiveState.ogList.toArray();
                m[i].args[1] = setLiveState.ogChildIdList.toArray();
		...
		m[i].args[4] = setLiveState.ogCIOTableList.toArray();
                ...
                m[i].threads = setLiveState.notifyThreads;
                m[i].type = Ding3dMessage.INSERT_NODES;
                m[i].universe = universe;
                m[i].args[0] = setLiveState.nodeList.toArray();
            }
	...
	if (boundingLeaf == null) {
		...
		Ding3dThread.UPDATE_RENDERING_ENVIRONMENT;
	    createMessage.type = Ding3dMessage.BACKGROUND_CHANGED;
	...
	super.doSetLive(s);
        ...
        if (inImmCtx) {
           throw new IllegalSharingException(
				Ding3dI18N.getString("BackgroundRetained1"));
        }
        if (inBackgroundGroup) {
            throw new
              IllegalSceneGraphException(Ding3dI18N.getString("BackgroundRetained5"));
        }

	if (inSharedGroup) {
	    throw new
		IllegalSharingException(Ding3dI18N.getString("BackgroundRetained6"));
	}
        ...
        if (geometryBranch != null) {
            ...
            if (branch.inBackgroundGroup == true)
               throw new IllegalSharingException(
				Ding3dI18N.getString("BackgroundRetained0"));

            if (branch.parent != null)
               throw new IllegalSharingException(
				Ding3dI18N.getString("BackgroundRetained3"));

            if (branch.locale != null)
               throw new IllegalSharingException(
				Ding3dI18N.getString("BackgroundRetained4"));
	...
	s.notifyThreads |= Ding3dThread.UPDATE_RENDERING_ENVIRONMENT|
	    Ding3dThread.UPDATE_RENDER;
