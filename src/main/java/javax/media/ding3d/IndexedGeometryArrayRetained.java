  ...
  int getIndexCount(){
	...
	if ((vertexFormat & GeometryArray.BY_REFERENCE) == 0) {
		...
		if ((vertexFormat & GeometryArray.INTERLEAVED) == 0) {
			...
			texBuffer = (FloatBufferWrapper)(((Ding3dBuffer) refTexCoordsBuffer[texCoordSet]).getBufferImpl());
	...
	if (dataValue < 0) {
	    ...
	    throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("IndexedGeometryArray27"));
    ...
    final void setCoordIndicesRef(int coordinateIndices[]) {
        ...
        if (coordinateIndices != null) {
            if (coordinateIndices.length < initialIndexIndex + validIndexCount) {
                throw new IllegalArgumentException(Ding3dI18N.getString("IndexedGeometryArray33"));
            }
	...
	if (validIndexCount < 0) {
	    throw new IllegalArgumentException(Ding3dI18N.getString("IndexedGeometryArray21"));
	}
	if ((initialIndexIndex + validIndexCount) > indexCount) {
	    throw new IllegalArgumentException(Ding3dI18N.getString("IndexedGeometryArray22"));
	}
