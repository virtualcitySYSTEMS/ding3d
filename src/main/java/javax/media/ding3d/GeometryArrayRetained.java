 ...
 *
 * $Revision: 1.13 $
 * $Date: 2008/12/17 13:50:47 $
 * $State: Exp $
 */

package javax.media.ding3d;

import javax.media.ding3d.internal.Distance;
...
abstract class GeometryArrayRetained extends GeometryRetained{
	...
	static final int targetThreads = (Ding3dThread.UPDATE_RENDER |
			Ding3dThread.UPDATE_GEOMETRY);
	...
	Ding3dBuffer coordRefBuffer = null;
	...
	Ding3dBuffer colorRefBuffer = null;
	...
	Ding3dBuffer normalRefBuffer = null;
	...
	Ding3dBuffer[] vertexAttrsRefBuffer = null;
	...
	Ding3dBuffer interleavedVertexBuffer = null;
	...
	void setLive(boolean inBackgroundGroup, int refCount) {
	    ...
	    createMessage.threads = Ding3dThread.UPDATE_RENDERING_ATTRIBUTES;
	    createMessage.type = Ding3dMessage.GEOMETRYARRAY_CHANGED;
	    ...
	    VirtualUniverse.mc.processMessage(createMessage);
			...
			if (this instanceof IndexedGeometryArrayRetained) {
				...
				Ding3dMessage createMessage = new Ding3dMessage();
				createMessage.threads = Ding3dThread.UPDATE_RENDERING_ATTRIBUTES;
				createMessage.type = Ding3dMessage.GEOMETRY_CHANGED;
				createMessage.universe = null;
	...
	void createGeometryArrayData(int vertexCount, int vertexFormat,
		...
		if ((vertexFormat & GeometryArray.BY_REFERENCE) == 0) {
			...
			if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0) {
				...
				this.refTexCoords = new Object[texCoordSetCount]; // keep Ding3dBufferImp object in nio buffer case
				if((vertexFormat & GeometryArray.USE_NIO_BUFFER) != 0 )
					this.refTexCoordsBuffer = new Object[texCoordSetCount]; // keep Ding3dBuffer object
			}
			if ((vertexFormat & GeometryArray.VERTEX_ATTRIBUTES) != 0) {
				...
				if ((vertexFormat & GeometryArray.USE_NIO_BUFFER) != 0) {
					this.vertexAttrsRefBuffer = new Ding3dBuffer[vertexAttrCount];
			...
			if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0) {
				...
				if ((src.vertexType & TEXCOORD_DEFINED) != 0) {
					for (index=start; index < end; index++) {
						for (i = 0, tOffset = vOffset;
								i < texCoordSetCount; i++) {
							texBuffer = (FloatBufferWrapper)(((Ding3dBuffer) (src.refTexCoordsBuffer[i])).getBufferImpl());
			...
			if (source != null && source.isLive()) {
				...
				synchronized (universeList) {
					...
					for (i = 0; i < numShapeMessages; i++, k++) {
						...
						m[k] = new Ding3dMessage();

						m[k].type = Ding3dMessage.GEOMETRY_CHANGED;
	...
	void setCoordRefBuffer(Ding3dBuffer coords) {
		if (coords != null) {
			switch (coords.getBufferType()) {
			case Ding3dBuffer.TYPE_FLOAT:
				assert ((FloatBufferWrapper)coords.getBufferImpl()).isDirect();
				break;
			case Ding3dBuffer.TYPE_DOUBLE:
				assert ((DoubleBufferWrapper)coords.getBufferImpl()).isDirect();
				break;
			case Ding3dBuffer.TYPE_NULL:
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray115"));

			default:
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray116"));
			}

			if (this instanceof IndexedGeometryArrayRetained) {
				IndexedGeometryArrayRetained idx = (IndexedGeometryArrayRetained)this;
				if (3 * idx.maxCoordIndex >= coords.getBufferImpl().limit()) {
					throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("IndexedGeometryArray23"));
				}
			} else if (coords.getBufferImpl().limit() < (3*(initialCoordIndex+validVertexCount))) {
				throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("GeometryArray99"));
			}
		}
	...
	Ding3dBuffer getCoordRefBuffer() {
	...
	void setCoordRefFloat(float[] coords) {
		...
		if (coords != null) {
			if ((vertexType & VERTEX_DEFINED) != 0 &&
				...
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray98"));
			}
	...
	void setColorRefFloat(float[] colors) {
		...
		if (colors != null) {
			...
			if ((vertexFormat & GeometryArray.COLOR) == 0) {
				throw new IllegalStateException(Ding3dI18N.getString("GeometryArray123"));
			}
			...
			if (this instanceof IndexedGeometryArrayRetained) {
				...
				if (getColorStride() * idx.maxColorIndex >= colors.length) {
					throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("IndexedGeometryArray24"));
				}
			} else  if (colors.length < getColorStride() * (initialColorIndex+ validVertexCount) ) {
				throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("GeometryArray112"));
			}
		}
	...
	Ding3dBuffer getColorRefBuffer() {
	...
	void setNormalRefFloat(float[] normals) {
		...
		if (normals != null) {
			...
			if ((vertexFormat & GeometryArray.NORMALS) == 0) {
				throw new IllegalStateException(Ding3dI18N.getString("GeometryArray122"));
			}
			...
			if (this instanceof IndexedGeometryArrayRetained) {
				...
				if (idx.maxNormalIndex*3 >= normals.length) {
					throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("IndexedGeometryArray26"));
				}
			} else if (normals.length < 3 * (initialNormalIndex + validVertexCount )) {
				throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("GeometryArray111"));
			}
		}
		boolean isLive = source!=null && source.isLive();
		if(isLive){
	...
	void setNormalRefBuffer(Ding3dBuffer normals) {

		FloatBufferWrapper bufferImpl = null;

		if (normals != null) {
			if(normals.getBufferType() != Ding3dBuffer.TYPE_FLOAT)
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray116"));
	...
	Ding3dBuffer getNormalRefBuffer() {
	...
	final int getTexStride() {
		...
		throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("GeometryArray121"));
	}
		...
		if (texCoordType != 0 && texCoordType != TF) {
			if (texCoords != null) {
				throw new IllegalArgumentException(
						Ding3dI18N.getString("GeometryArray98"));
			}
			return;
		}
		...
		if (texCoords != null) {
			...
			if (this instanceof IndexedGeometryArrayRetained) {
				...
				if (idx.maxTexCoordIndices[texCoordSet]*ts >= texCoords.length) {
					throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("IndexedGeometryArray25"));
				}
			} else if (texCoords.length < ts*(initialTexCoordIndex[texCoordSet]+validVertexCount)) {
				throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("GeometryArray113"));
			}
		}
		boolean isLive = source!=null && source.isLive();
		if(isLive){
	...
	void setTexCoordRefBuffer(int texCoordSet, Ding3dBuffer texCoords) {

		FloatBufferWrapper bufferImpl = null;

		if (texCoords != null) {
			if(texCoords.getBufferType() != Ding3dBuffer.TYPE_FLOAT)
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray116"));
		...
		dirtyFlag |= TEXTURE_CHANGED;
		// refTexCoordsBuffer contains Ding3dBuffer object for tex coord
	...
	Ding3dBuffer getTexCoordRefBuffer(int texCoordSet) {
		return (Ding3dBuffer)(refTexCoordsBuffer[texCoordSet]);
	}
		...
		if (texCoords != null) {
			if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE_2) == 0) {
				throw new IllegalStateException(
						Ding3dI18N.getString("GeometryArray94"));
			}
	...
	void setTexCoordRef3f(int texCoordSet, TexCoord3f[] texCoords) {
		...
		if (texCoords != null) {
			...
			if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE_3) == 0) {
				throw new IllegalStateException(
						Ding3dI18N.getString("GeometryArray95"));
			}
	...
	void setVertexAttrRefFloat(int vertexAttrNum, float[] vertexAttrs) {
        ...
        if (vertexAttrType != 0 && vertexAttrType != AF) {
            if (vertexAttrs != null) {
                ...
                throw new IllegalArgumentException(
                        Ding3dI18N.getString("GeometryArray98-XXX"));
            }
            return;
        }
		 */
		...
		if (vertexAttrs != null) {
			...
			if (this instanceof IndexedGeometryArrayRetained) {
				...
				if (sz*idx.maxVertexAttrIndices[vertexAttrNum] >= vertexAttrs.length) {
					throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("IndexedGeometryArray30"));
				}

			} else if (vertexAttrs.length < sz*(initialVertexAttrIndex[vertexAttrNum] + validVertexCount) ) {
				throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("GeometryArray129"));
			}
		}
		boolean isLive = source!=null && source.isLive();
		if(isLive){
	 ...
	 */
	void setVertexAttrRefBuffer(int vertexAttrNum, Ding3dBuffer vertexAttrs) {

		FloatBufferWrapper bufferImpl = null;

		if (vertexAttrs != null) {
			if(vertexAttrs.getBufferType() != Ding3dBuffer.TYPE_FLOAT)
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray116"));
	 ...
	 * vertex attribute number.
	 */
	Ding3dBuffer getVertexAttrRefBuffer(int vertexAttrNum) {
		...
		if (vertexData != null) {
			...
			if (this instanceof IndexedGeometryArrayRetained) {
					...
					for (int i = 0; i < texCoordSetCount; i++) {
						if (stride * idx.maxTexCoordIndices[i] >= vertexData.length) {
							throw new ArrayIndexOutOfBoundsException(
									Ding3dI18N.getString("IndexedGeometryArray25"));
						}
					}
				}
				...
				if (vertexData.length < (stride * (initialVertexIndex+validVertexCount)))
					throw new ArrayIndexOutOfBoundsException(Ding3dI18N.getString("GeometryArray114"));
			}
		}
	...
	void setInterleavedVertexBuffer(Ding3dBuffer vertexData) {

		FloatBufferWrapper bufferImpl = null;

		if (vertexData != null ){

			if (vertexData.getBufferType() != Ding3dBuffer.TYPE_FLOAT)
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray116"));
	...
	Ding3dBuffer getInterleavedVertexBuffer() {
	...
	void setValidVertexCount(int validVertexCount) {
		...
		if (validVertexCount < 0) {
			throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray110"));
		}

		if ((initialVertexIndex + validVertexCount) > vertexCount) {
			throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray100"));
		}
		...
		if ((vertexFormat & GeometryArray.INTERLEAVED) != 0) {
			...
			if ((initialCoordIndex + validVertexCount) > vertexCount) {
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray104"));
			}
			if ((initialColorIndex + validVertexCount) > vertexCount) {
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray101"));
			}
			if ((initialNormalIndex + validVertexCount) > vertexCount) {
				throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray102"));
			}

			if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0) {
				for (int i = 0; i < texCoordSetCount; i++) {
					if ((initialTexCoordIndex[i] + validVertexCount) > vertexCount) {
						throw new IllegalArgumentException(Ding3dI18N.getString(
								"GeometryArray103"));
					}
				}
			}
			...
			if (( vertexFormat & GeometryArray.USE_NIO_BUFFER) != 0) {
				...
				case TF:
					FloatBufferWrapper texBuffer;
					for (int i = 0; i < texCoordSetCount; i++) {
						texBuffer = (FloatBufferWrapper)(((Ding3dBuffer)refTexCoordsBuffer[i]).getBufferImpl());
						if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE_2) != 0) {
							if (texBuffer.limit() <  2 * (initialTexCoordIndex[i] + validVertexCount) ) {
								throw new ArrayIndexOutOfBoundsException(
										Ding3dI18N.getString("GeometryArray113"));
							}
				...
				case AF:
					for (int i = 0; i < vertexAttrCount; i++) {
						...
						if (floatBufferRefVertexAttrs[i].limit() <
							...
							throw new ArrayIndexOutOfBoundsException(
									Ding3dI18N.getString("GeometryArray129"));
						}
					}
					break;
				}
	...
	void setInitialVertexAttrIndex(int vertexAttrNum,
		...
		if ((initialVertexAttrIndex + validVertexCount) > vertexCount) {
			throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray130"));
		}
	...
	void setInitialTexCoordIndex(int texCoordSet, int initialTexCoordIndex) {
		if ((initialTexCoordIndex + validVertexCount) > vertexCount) {
			throw new IllegalArgumentException(Ding3dI18N.getString("GeometryArray103"));
		}

		if((vertexFormat & GeometryArray.USE_NIO_BUFFER) != 0){
			if((vertexType & TEXCOORD_DEFINED) == TF) {
				FloatBufferWrapper texBuffer = (FloatBufferWrapper)(((Ding3dBuffer) refTexCoordsBuffer[texCoordSet]).getBufferImpl());
				if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE_2) != 0) {
	...
	int getNumTexCoordCount(int i) {
		...
		if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE) != 0){
			...
			if ((vertexFormat & GeometryArray.USE_NIO_BUFFER) == 0) {
				...
				if ((vertexFormat & GeometryArray.INTERLEAVED) == 0){
					if ((vertexType & TEXCOORD_DEFINED) == TF) {
						FloatBufferWrapper texBuffer = (FloatBufferWrapper)(((Ding3dBuffer) refTexCoordsBuffer[i]).getBufferImpl());
						if ((vertexFormat & GeometryArray.TEXTURE_COORDINATE_2) != 0) {
