...
public class Sensor {
    ...
    public void setPredictor(int predictor){
       if (predictor != PREDICT_NONE && predictor != PREDICT_NEXT_FRAME_TIME) {
           throw new IllegalArgumentException(Ding3dI18N.getString("Sensor0"));
       } else {
	...
	if (policy != NO_PREDICTOR && policy != HEAD_PREDICTOR &&
	    ...
	    throw new IllegalArgumentException(Ding3dI18N.getString("Sensor1"));
	else
   ...
   throw new IllegalArgumentException(Ding3dI18N.getString("Sensor3"));
        }
    ...
    public long lastTime(int k){
        if(k >= sensorReadCount) {
           throw new IllegalArgumentException(Ding3dI18N.getString("Sensor4"));
        }
