...
abstract class FogRetained extends LeafRetained{
  ...
  void removeAllScopes() {
	...
	TransformGroupRetained[] tlist;
        ...
        if (inImmCtx) {
	    throw new IllegalSharingException(Ding3dI18N.getString("FogRetained0"));
        }
        super.doSetLive(s);

	if (inSharedGroup) {
	    throw new
		IllegalSharingException(Ding3dI18N.getString("FogRetained1"));
	}
   ...
   // This is called on the parent object
	...
	if (scopes.size() > 0) {
	    ...
	    createMessage.type = Ding3dMessage.FOG_CHANGED;
