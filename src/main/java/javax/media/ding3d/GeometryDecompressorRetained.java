...
class GeometryDecompressorRetained extends GeometryDecompressor {
	...
	switch(bufferDataType) {
	  ...
	  default:
		...
		(Ding3dI18N.getString("GeometryDecompressorRetained0")) ;
	}
	...
	if (vlist != null)
		...
		(Ding3dI18N.getString("GeometryDecompressorRetained1")) ;
	...
	if ((vlist.vertexFormat & GeometryArray.COLOR) == 0) {
	    if (vlist.size() > 0)
		throw new IllegalStateException
		    (Ding3dI18N.getString("GeometryDecompressorRetained2")) ;
	...
	if ((vlist.vertexFormat & GeometryArray.NORMALS) == 0) {
	    if (vlist.size() > 0)
		throw new IllegalStateException
		    (Ding3dI18N.getString("GeometryDecompressorRetained3")) ;
	...
	switch(bufferDataType) {
	  ...
	  default:
	    throw new IllegalArgumentException
		(Ding3dI18N.getString("GeometryDecompressorRetained4")) ;
	}
	...
	startTime = Ding3dClock.currentTimeMillis() ;
    }

    private void endPrint() {
	endTime = Ding3dClock.currentTimeMillis() ;
