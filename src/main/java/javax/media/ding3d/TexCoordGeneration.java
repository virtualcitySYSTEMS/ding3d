...
public class TexCoordGeneration extends NodeComponent {
   ...
   // Array for setting default read capabilities
    ...
    public void setEnable(boolean state) {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_ENABLE_WRITE))
              throw new CapabilityNotSetException(Ding3dI18N.getString("TexCoordGeneration0"));
	if (isLive())
    ...
    public boolean getEnable() {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_ENABLE_READ))
              throw new CapabilityNotSetException(Ding3dI18N.getString("TexCoordGeneration1"));
    ...
    public int getFormat() {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_FORMAT_READ))
              throw new CapabilityNotSetException(Ding3dI18N.getString("TexCoordGeneration2"));
	...
	if ((genMode < OBJECT_LINEAR) || (genMode > REFLECTION_MAP)) {
		...
		Ding3dI18N.getString("TexCoordGeneration5"));
	}
	((TexCoordGenerationRetained)this.retained).initGenMode(genMode);
    }
    ...
    public int getGenMode() {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_MODE_READ))
              throw new CapabilityNotSetException(Ding3dI18N.getString("TexCoordGeneration3"));
