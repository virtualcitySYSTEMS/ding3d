...
public class Group extends Node {
	...
	if (isLiveOrCompiled())
		...
		throw new CapabilityNotSetException(Ding3dI18N.getString("Group0"));
	...
	if (isLiveOrCompiled())
		...
		throw new CapabilityNotSetException(Ding3dI18N.getString("Group1"));
	...
	if (isLiveOrCompiled()) {
		...
		throw new RestrictedAccessException(Ding3dI18N.getString("Group3"));

	    if (!getCapability(ALLOW_CHILDREN_WRITE))
		throw new CapabilityNotSetException(Ding3dI18N.getString("Group13"));

	    if ((oldchild != null) &&
		(! ((BranchGroup)oldchild).getCapability(BranchGroup.ALLOW_DETACH))) {
		throw new CapabilityNotSetException(Ding3dI18N.getString("Group4"));
	    }
	}
	...
	if (isLiveOrCompiled()) {
	    if (! (child instanceof BranchGroup))
		throw new RestrictedAccessException(Ding3dI18N.getString("Group6"));

	    if (!this.getCapability(ALLOW_CHILDREN_WRITE))
		throw new CapabilityNotSetException(Ding3dI18N.getString("Group14"));
	}
	...
	if (isLiveOrCompiled()) {
	    ...
	    if (!(child instanceof BranchGroup)) {
		throw new RestrictedAccessException(Ding3dI18N.getString("Group7"));
	    }

	    if (!this.getCapability(ALLOW_CHILDREN_WRITE)) {
		throw new CapabilityNotSetException(Ding3dI18N.getString("Group15"));
	    }
	...
	if (isLiveOrCompiled())
		...
		throw new CapabilityNotSetException(Ding3dI18N.getString("Group9"));
