...
class GroupRetained extends NodeRetained implements BHLeafInterface {
	...
	if (this.source.isLive()) {
	    ...
	    messages = new Ding3dMessage[numMessages];
	    for (int i = 0; i < numMessages; i++) {
	         messages[i] = new Ding3dMessage();
	    }
	}
    ...
    // The method that does the work once the lock is acquired.
    void doRemoveChild(int index, Ding3dMessage messages[], int messageIndex) {
	...
	if ((child != null) &&
		...
		throw new MultipleParentException(Ding3dI18N.getString(s));
	}
    }
    ...
    // The method that does the work once the lock is acquired.
    void doAddChild(Node child, Ding3dMessage messages[], int messageIndex) {
	...
	if(branchGroup != null) {
	    ...
	    for (int i=0; i<numMessages; i++) {
		messages[i] = new Ding3dMessage();
		messages[i].type = Ding3dMessage.INVALID_TYPE;
	    }
	...
	if (numMessages > 0) {
	    int count = 0;
	    for (int i=0; i < numMessages; i++) {
		if (messages[i].type != Ding3dMessage.INVALID_TYPE) {
		    count++;
		}
	    }
		...
		Ding3dMessage ms[] = null;

		if (count > 0) {
		    ms = new Ding3dMessage[count];
		}
   ...
   synchronized void setAltAppScope() {
	...
	if (source.isLive()) {
	    ...
	    Ding3dMessage message = new Ding3dMessage();
	    message.threads = Ding3dThread.UPDATE_GEOMETRY;
	    message.universe = universe;
		...
		message.type = Ding3dMessage.INSERT_NODES;
		...
		message.type = Ding3dMessage.REMOVE_NODES;
		message.args[0] = mirrorGroup.toArray();
                message.args[1] = ti;
                message.args[2] = newCtArr;
     ...
     */
    void checkSetLive(NodeRetained child, int childIndex, Ding3dMessage messages[],
    ...
    void checkSetLive(NodeRetained child, int childIndex, HashKey keys[],
		      boolean isShared, Ding3dMessage messages[],
		      int messageIndex, NodeRetained linkNode) {

        SceneGraphObject me = this.source;
	SetLiveState s;
	Ding3dMessage createMessage;
	...
	boolean sendVSGMessage = true;
		...
		for(int mIndex=0; mIndex < numMessages; mIndex++) {
		    messages[mIndex] = new Ding3dMessage();
		}
		sendMessages = true;
	    }
		...
		createMessage.type = Ding3dMessage.ORDERED_GROUP_INSERTED;
		...
		createMessage.type = Ding3dMessage.VIEWSPECIFICGROUP_INIT;
		createMessage.universe = universe;
	    ...
	    createMessage.type = Ding3dMessage.INSERT_NODES;
	    createMessage.universe = universe;
	    ...
	    createMessage.threads = Ding3dThread.UPDATE_BEHAVIOR;
	    createMessage.type = Ding3dMessage.BEHAVIOR_ACTIVATE;
	    createMessage.universe = universe;
    ...
    void checkClearLive(NodeRetained child,
                        Ding3dMessage messages[], int messageIndex,
	...
	TransformGroupRetained tg;
		...
		destroyMessage.threads = Ding3dThread.UPDATE_RENDER |
		    Ding3dThread.UPDATE_RENDERING_ENVIRONMENT;
		destroyMessage.type = Ding3dMessage.ORDERED_GROUP_REMOVED;
	    ...
	    destroyMessage = messages[messageIndex++];
            ...
            destroyMessage.type = Ding3dMessage.REMOVE_NODES;
            destroyMessage.universe = universe;
	    ...
	    if(sendVSGMessage) {
		destroyMessage = messages[messageIndex++];
		destroyMessage.threads =  Ding3dThread.UPDATE_RENDERING_ENVIRONMENT;
		destroyMessage.type = Ding3dMessage.VIEWSPECIFICGROUP_CLEAR;
		destroyMessage.universe = universe;
