...
class ImageComponent3DRetained extends ImageComponentRetained {
    ...
    void set(int index, NioImageBuffer nioImage) {
        ...
        if (!byReference) {
            throw new IllegalArgumentException(Ding3dI18N.getString("Need_New_Message_XXXXXImageComponent2D7"));
        }
        if (!yUp) {
            throw new IllegalArgumentException(Ding3dI18N.getString("Need_New_Message_XXXXXImageComponent2D8"));
        }
	    ...
	    for (i=0; i<depth; i++) {
            ...
            for (i = 0; i < depth; i++) {
                ...
                if (!(bi[i] instanceof BufferedImage)) {
                    throw new IllegalStateException(Ding3dI18N.getString("ImageComponent3DRetained0"));
                }
    ...
    void setSubImage(int index, RenderedImage image, int width, int height,
         ...
         if(!isSubImageTypeEqual(image)) {
            throw new IllegalStateException(
                                Ding3dI18N.getString("ImageComponent2D6"));
        }
