...
import  javax.media.ding3d.vecmath.Point4d;
...
public class SceneGraphPath {
    ...
    public final void setNode(int index, Node newNode) {
      if(interior == null)
	throw new NullPointerException(Ding3dI18N.getString("SceneGraphPath0"));
    ...
    public final Node getNode(int index) {
      if(interior == null)
	throw new
          ArrayIndexOutOfBoundsException(Ding3dI18N.getString("SceneGraphPath1"));
   ...
   /**
	...
	if(item != null) {
	    ...
	    try {
		if (item.getClass().getName().equals("javax.media.ding3d.Shape3D"))
	...
	if (interior != null) {
		...
		if (!node.isLive()) {
		    throw new RuntimeException(Ding3dI18N.getString("SceneGraphPath3"));
		}
	...
	if( root == null )
	    throw new IllegalArgumentException(Ding3dI18N.getString("SceneGraphPath2"));

	if( item == null )
	    throw new IllegalArgumentException(Ding3dI18N.getString("SceneGraphPath10"));

	// verify liveness
	if( !item.isLive() )
	    throw new IllegalArgumentException(Ding3dI18N.getString("SceneGraphPath3"));
	...
	if(interior != null) {
		...
		if(currentNR == null  &&  bottomNR instanceof SharedGroupRetained) {
			...
			throw new IllegalArgumentException(Ding3dI18N.getString("SceneGraphPath5"));

		}

		while(currentNR != nextNR) {
		    if(currentNR == null) {
			throw new IllegalArgumentException(Ding3dI18N.getString("SceneGraphPath11"));
		    }
	...
	if(!root.branchGroups.contains(currentNode)) {
	    throw new IllegalArgumentException(Ding3dI18N.getString("SceneGraphPath9"));
	}
