...
abstract class TextureRetained extends NodeComponentRetained {
	...
	if (this.images == null) {
           throw new IllegalArgumentException(Ding3dI18N.getString("TextureRetained0"));
	}

        if (this.source instanceof Texture2D) {
            if (image instanceof ImageComponent3D)
               throw new IllegalArgumentException(Ding3dI18N.getString("Texture8"))
;
        } else {

            if (image instanceof ImageComponent2D)
               throw new IllegalArgumentException(Ding3dI18N.getString("Texture14")
);
        }
    ...
    final void checkImageSize(int level, ImageComponent image) {
	    ...
	    if ((wdh != (imgWidth - 2*boundaryWidth)) ||
	       ...
	       throw new IllegalArgumentException(
				Ding3dI18N.getString("TextureRetained1"));
	    }
        }
    }
	...
	if ((level < 0) || (level > maximumLevel)) {
	    throw new IllegalArgumentException(
			Ding3dI18N.getString("Texture36"));
	}
	baseLevel = level;
    }
    ...
    final void initMaximumLevel(int level) {
        if ((level < baseLevel) || (level >=  maxMipMapLevels)) {
            throw new IllegalArgumentException(
                    Ding3dI18N.getString("Texture37"));
        }
        if((mipmapMode == Texture.BASE_LEVEL) && (level != 0)) {
            throw new IllegalArgumentException(
                    Ding3dI18N.getString("Texture48"));
        }
	...
	if (lod > maximumLod) {
	    throw new IllegalArgumentException(
			Ding3dI18N.getString("Texture42"));
	}
	minimumLod = lod;
    }
	...
	if (images != null) {
	    ...
	    for (int j = 0; j < numFaces; j++) {
	  	 for (int i = 0; i < maxLevels; i++){
	    	      if (images[j][i] == null) {
              		  throw new IllegalArgumentException(
				Ding3dI18N.getString("TextureRetained3") + i);
		      }
	...
	if (images != null) {
        ...
        createMessage.type = Ding3dMessage.TEXTURE_CHANGED;
