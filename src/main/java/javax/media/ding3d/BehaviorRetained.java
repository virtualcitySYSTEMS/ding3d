...
class BehaviorRetained extends LeafRetained  {
	...
	if (source != null && source.isLive()) {
	    sendMessage(Ding3dMessage.REGION_BOUND_CHANGED);
	}
    }
	...
	if (enable != state) {
	    enable = state;
	    if (source != null && source.isLive()) {
		sendMessage(state ? Ding3dMessage.BEHAVIOR_ENABLE:
			            Ding3dMessage.BEHAVIOR_DISABLE);
	    }
	}
    }
	...
	if ((source != null) && source.isLive()
	    ...
	    sendMessage(Ding3dMessage.SCHEDULING_INTERVAL_CHANGED,
   ...
   */
	...
	if (inBackgroundGroup) {
	    throw new
		IllegalSceneGraphException(Ding3dI18N.getString("BehaviorRetained0"));
	}
	if (inSharedGroup) {
	    throw new
		IllegalSharingException(Ding3dI18N.getString("BehaviorRetained1"));
	}

	s.nodeList.add(this);
        s.behaviorNodes.add(this);
	s.notifyThreads |= Ding3dThread.UPDATE_BEHAVIOR;
        // process switch leaf
    ...
    */
	...
	// handleLastWakeupOn()
	if (criteria == null) {
	    throw new NullPointerException(Ding3dI18N.getString("BehaviorRetained2"));
	}
