...
//
                    ...
                    for(int i=0; i<images.length; i++) {
                        if(images[i] != null) {
                            ...
                            if(imageRetained.getUsedByOffScreen()) {
                                throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D30"));
                            }
                        }
                    }
                }
    ...
    public void setBackground(Background background) {
        ...
        if (background.isLive()) {
           throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D11"));
        }
        ...
        if(image != null) {
            ...
            if(imageRetained.getUsedByOffScreen()) {
                throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D31"));
            }
        }

        if (((BackgroundRetained)background.retained).geometryBranch != null) {
           throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D22"));
        }
    ...
    public void setFog(Fog fog) {
        if (fog != null && fog.isLive()) {
           throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D12"));
        }
        uFog = fog;
        if ((canvas3d.view == null) ||
	    (canvas3d.view.universe == null) ||
    ...
    public void setModelClip(ModelClip modelClip) {
        if ((modelClip != null) && modelClip.isLive()) {
           throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D25"));
        }
    ...
    public void setLight(Light light, int index) {
        if (light == null) {
           throw new NullPointerException(Ding3dI18N.getString("GraphicsContext3D13"));
        }
        if (light.isLive()) {
           throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D14"));
        }
    ...
    public void setSound(Sound sound, int index) {
        if (sound == null) {
           throw new NullPointerException(Ding3dI18N.getString("GraphicsContext3D17"));
        }
        if (sound.isLive()) {
           throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D23"));
        }
...
//
	...
	if (canvas3d.ctx == null) {
        ...
        if (Ding3dDebug.devPhase && Ding3dDebug.debug) {
            Ding3dDebug.doAssert(canvas3d.ctx != null, "canvas3d.ctx != null");
        }
...
//  	    vpcToEc = cvCache.getLeftVpcToEc();
    ...
    public void draw(Geometry geometry) {
        ...
        if((geometry != null) && (geometry instanceof Raster)) {
            ...
            if(image != null) {
                ...
                // Do illegal sharing check
                if(imageRetained.getUsedByOffScreen()) {
                    throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D32"));
                }
            }
        }
	...
	if (shape.isLive()) {
	    throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D26"));
	}
        ...
        if ((raster != null) && raster.isLive()) {
            ...
            if (image != null){
                ...
                if (image.getImageClass() != ImageComponent.ImageClass.BUFFERED_IMAGE) {
                    throw new IllegalArgumentException(Ding3dI18N.getString("GraphicsContext3D33"));
                }
                if (image.isByReference() && (image.getImage() == null)) {
                    throw new IllegalArgumentException(Ding3dI18N.getString("GraphicsContext3D34"));
                }
                if (imageRetained.getNumberOfComponents() < 3) {
                    throw new IllegalArgumentException(Ding3dI18N.getString("GraphicsContext3D35"));
                }
                if (image.isLive()) {
                    throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D36"));
                }
                if (imageRetained.getInImmCtx() || imageRetained.getUsedByOffScreen()) {
                    throw new IllegalSharingException(Ding3dI18N.getString("GraphicsContext3D37"));
                }
            }
        }
        if ((canvas3d.view == null) || (canvas3d.view.universe == null) ||
    ...
    void doReadRaster(Raster raster) {
        ...
        if ( (ras.type & Raster.RASTER_COLOR) != 0) {
            if ((rasterSize.width > ras.image.width) ||
                ...
                throw new RuntimeException(Ding3dI18N.getString("GraphicsContext3D27"));
            }
        }
        ...
        if ( (ras.type & Raster.RASTER_DEPTH) != 0) {
            ...
            if ((rasterSize.width > ras.depthComponent.width) ||
                ...
                throw new RuntimeException(Ding3dI18N.getString("GraphicsContext3D28"));
            }
        }
	...
	try {
	  if (canvas3d.drawingSurfaceObject.renderLock()) {
	      ...
	      canvas3d.drawingSurfaceObject.unLock();
	      if (wait) {
		  runMonitor(Ding3dThread.NOTIFY);
	      }
	  }
	} catch (NullPointerException ne) {
	    canvas3d.drawingSurfaceObject.unLock();
	...
	while (!canvas3d.view.inRenderThreadData) {
        ...
        if (renderRun) {
            ...
            VirtualUniverse.mc.sendRunMessage(canvas3d.view, Ding3dThread.RENDER_THREAD);
        } else {
    ...
    void sendSoundMessage(int command, Object arg1, Object arg2) {
        ...
        Ding3dMessage soundMessage = new Ding3dMessage();
        soundMessage.threads = Ding3dThread.SOUND_SCHEDULER;
        soundMessage.type = Ding3dMessage.RENDER_IMMEDIATE;
