...
/**
 * The Ding3dThread is the super class of all slave threads in Java 3D.  It implements
 * all of the common flow control constructs.
 */

abstract class Ding3dThread extends Thread {
    /**
...
//  static final int COLLISION_THREAD       		= 0x20;
     ...
     */
    private Ding3dThreadData[] data = null;
     ...
     */
    synchronized Ding3dThreadData getThreadData(View v, Canvas3D c) {
	Ding3dThreadData threadData;
	int i, j;
	Ding3dThreadData[] newData;

	if (type != RENDER_THREAD) { // Regular Thread
	    if (data == null) {
		data = new Ding3dThreadData[1];
		data[0] = new Ding3dThreadData();
		data[0].thread = this;
	...
	// Note: each renderer has multiple thread data mappings
		...
		if (i==data.length) {
		    newData = new Ding3dThreadData[data.length+1];
		    for (j=0; j<data.length; j++) {
			newData[j] = data[j];
		    }
		    data = newData;
		    data[j] = new Ding3dThreadData();
		    data[j].thread = this;
