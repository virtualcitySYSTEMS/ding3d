...
class TransformStructure extends Ding3dStructure implements ObjectUpdate {
	...
	super(u, Ding3dThread.UPDATE_TRANSFORM);
    }
	...
	synchronized (universe.sceneGraphLock) {
	    ...
	    for (i = (nMsg-1); i >= 0; i--) {
		m = messages[i];
		if (m.type == Ding3dMessage.TRANSFORM_CHANGED) {
		...
		case Ding3dMessage.REGION_BOUND_CHANGED:
		    ...
		    break;
		case Ding3dMessage.COLLISION_BOUND_CHANGED:
	...
	if (Ding3dDebug.devPhase && Ding3dDebug.debug) {
	    Ding3dDebug.doDebug(Ding3dDebug.transformStructure, Ding3dDebug.LEVEL_5,
    ...
    void processRegionBoundChanged(Ding3dMessage m) {
    ...
    void processCollisionBoundChanged(Ding3dMessage m) {
	int i;
        Shape3DRetained ms;
    ...
    void processBoundsAutoComputeChanged(Ding3dMessage m) {
        // need to update mirrorShape's bounds
        processBoundsChanged((Object[]) m.args[0], (Bounds) m.args[1]);
    }
