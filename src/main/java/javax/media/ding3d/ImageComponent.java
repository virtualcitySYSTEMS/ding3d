   ...
   */
   ...
   // Array for setting default read capabilities
    ...
    public int getWidth() {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_SIZE_READ))
              throw new CapabilityNotSetException(Ding3dI18N.getString("ImageComponent0"));
    ...
    public int getHeight() {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_SIZE_READ))
              throw new CapabilityNotSetException(Ding3dI18N.getString("ImageComponent1"));
    ...
    public int getFormat() {
        if (isLiveOrCompiled())
            if(!this.getCapability(ALLOW_FORMAT_READ))
              throw new CapabilityNotSetException(Ding3dI18N.getString("ImageComponent2"));
